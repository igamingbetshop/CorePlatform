...
using IqSoft.CP.Integration.EcopayzServiceReferenceProd;
    ...
    public static class EcopayzHelpers
    {
        ...
        public static PaymentResponse CreatePayoutRequest(PaymentRequest input, SessionIdentity session, ILog log)
        {
            using (var paymentSystemBl = new PaymentSystemBll(session, log))
            {
                ...
                var merchantAPIServiceSoapClient = new MerchantAPIServiceSoapClient();
                var res = merchantAPIServiceSoapClient.Payout(payoutInput);
                if (res.ErrorCode == 0)
                {
