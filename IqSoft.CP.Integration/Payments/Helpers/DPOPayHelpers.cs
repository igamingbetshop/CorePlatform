                ...
                using (var notificationBl = new NotificationBll(clientBl))
                {
                    ...
                    using (Stream stream = CommonFunctions.SendHttpRequestForStream(httpRequestInput, out _, SecurityProtocolType.Tls12))
                    {
                        ...
                        if (verifyOutput.Result == "000")
                        {
                            ...
                            clientBl.ChangeDepositRequestState(paymentRequest.Id, PaymentRequestStates.Expired, verifyOutput.ResultExplanation, notificationBl);
                        else
                            throw new Exception(JsonConvert.SerializeObject(verifyOutput));
                    }
                }
            }
            return userIds;
