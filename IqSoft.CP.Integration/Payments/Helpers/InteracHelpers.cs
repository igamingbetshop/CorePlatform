    ...
    public static class InteracHelpers
    {
        public static string CallInteracApi(PaymentRequest input, string cashierPageUrl, SessionIdentity session, ILog log)
        {
            ...
            using (var paymentSystemBl = new PaymentSystemBll(session, log))
            {
                ...
                if (input.CurrencyId != Constants.Currencies.CanadianDollar)
                {
                    var rate = BaseBll.GetCurrenciesDifference(client.CurrencyId, Constants.Currencies.CanadianDollar);
                    amount = Math.Round(rate * input.Amount, 0);
                    ...
                    parameters.Add("Currency", Constants.Currencies.CanadianDollar);
                ...
                var paymentInput = new
                {
                    ...
                    site = $"https://{session.Domain}",
                    userIp = session.LoginIp,
                    currency = Constants.Currencies.CanadianDollar,
                    amount = (int)amount,
        ...
        public static PaymentResponse CreatePayoutRequest(PaymentRequest paymentRequest, SessionIdentity session, ILog log)
        {
            using (var paymentSystemBl = new PaymentSystemBll(session, log))
            {
                ...
                if (paymentRequest.CurrencyId != Constants.Currencies.CanadianDollar)
                {
                    var rate = BaseBll.GetCurrenciesDifference(client.CurrencyId, Constants.Currencies.CanadianDollar);
                    amount = Math.Round(rate * paymentRequest.Amount, 0);
                ...
                httpRequestInput.RequestMethod = Constants.HttpRequestMethods.Post;
