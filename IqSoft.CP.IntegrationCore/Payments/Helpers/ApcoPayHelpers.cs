			...
			using (var paymentSystemBl = new PaymentSystemBll(session, log))
			{
				using (var currencyBll = new CurrencyBll(paymentSystemBl))
				{
					...
					var transaction = new Transaction
					{
						...
						status_url = string.Format("{0}/api/ApcoPay/ApiRequest", CacheManager.GetPartnerSettingByKey(client.PartnerId, Constants.PartnerKeys.PaymentGateway).StringValue),
				...
				using (var paymentSystemBl = new PaymentSystemBll(partnerBl))
				{
					using (var currencyBl = new CurrencyBll(partnerBl))
					{
						...
						var transaction = new Transaction
						{
							...
							status_url = string.Format("{0}/{1}", partnerBl.GetPaymentValueByKey(client.PartnerId, null, Constants.PartnerKeys.PaymentGateway), "api/ApcoPay/ApiRequest"),
							return_pspid = string.Empty,
