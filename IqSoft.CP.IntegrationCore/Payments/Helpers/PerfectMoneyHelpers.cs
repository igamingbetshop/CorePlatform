...
namespace IqSoft.CP.Integration.Payments.Helpers
{
    public class PerfectMoneyHelpers
    {
        ...
        public static string CallPerfectMoneyApi(PaymentRequest input, string cashierPageUrl, SessionIdentity session, ILog log)
        {
            ...
            var segment = clientBl.GetClientPaymentSegments(input.ClientId, partnerPaymentSetting.PaymentSystemId).OrderBy(x => x.Priority).FirstOrDefault();
            var link = segment == null ? CacheManager.GetPartnerSettingByKey(null, Constants.PartnerKeys.DistributionUrl).StringValue : segment.ApiUrl;
