...
namespace IqSoft.CP.BetShopGatewayWebApi.Helpers
{
    public static class Helpers
    {
        ...
        public static CashDeskData CheckEncodedData(int partnerId, string externalId, string hash, IBetShopBll betShopBl)
        {
            if (string.IsNullOrWhiteSpace(hash))
                throw BaseBll.CreateException(string.Empty, Constants.Errors.WrongHash);
            var cashDesk = betShopBl.GetCashDeskByMacAddress(partnerId, externalId);
            if (cashDesk == null)
            ...
            if (betShop.PartnerId != partnerId)
                throw BaseBll.CreateException(string.Empty, Constants.Errors.WrongPartnerId);
            var encryption = new RijndaelEncrypt(cashDesk.EncryptPassword, cashDesk.EncryptSalt, cashDesk.EncryptIv);

            var cashDeskDataStr = encryption.Decrypt(hash);
            var data = JsonConvert.DeserializeObject<CashDeskData>(cashDeskDataStr);

            if(data.MacAddress != cashDesk.MacAddress)
                throw BaseBll.CreateException(string.Empty, Constants.Errors.WrongHash);
