...
namespace IqSoft.CP.AdminWebApi.Controllers
{
    ...
    public class CRMController : ControllerBase
    {
        ...
        public HttpResponseMessage CRMData(ApiRequestInput request)
        {
           var actionLog = new DAL.ActionLog
           {
            ...
            try
            {
                ...
                apiResponseBase = CallFunction(request, identity, Program.DbLogger);

            }
            catch (FaultException<BllFnErrorType> fex)
            {
                Program.DbLogger.Error(string.Format(logTemplate, fex.Detail.Id, fex.Detail.Message));
                apiResponseBase.ResponseCode = fex.Detail.Id;
                apiResponseBase.Description = fex.Detail.Message;
            }
            catch (Exception ex)
            {
                Program.DbLogger.Error(string.Format(logTemplate, Constants.Errors.GeneralException, ex.Message));
        ...
        private DAL.ActionLog CheckAuthentication(ApiRequestInput request, int partnerId)
        {
            ...
            var ip = Request.Headers.ContainsKey("CF-Connecting-IP") ? Request.Headers["CF-Connecting-IP"].ToString() :
            ...
            actionLog.Ip = ip;
            actionLog.Domain = Request.Headers.ContainsKey("Origin") ? Request.Headers["Origin"].ToString() : Request.Host.ToString();
        ...
        public static ApiResponseBase GetPaymentRequests(ApiBaseFilter apiFilter, SessionIdentity identity, ILog log)
        {
            ...
            var startDate = apiFilter.StartDate.Value.Year * 1000000 + apiFilter.StartDate.Value.Month * 10000 +
                            apiFilter.StartDate.Value.Day * 100 + apiFilter.StartDate.Value.Hour;
            ...
            if (apiFilter.EndDate.HasValue)
                filter.ToDate = apiFilter.EndDate.Value.Year * 1000000 + apiFilter.EndDate.Value.Month * 10000 +
                                apiFilter.EndDate.Value.Day * 100 + apiFilter.EndDate.Value.Hour;
            return new ApiResponseBase
            {
        ...
        {
            using var paymentSystemBl = new PaymentSystemBll(identity, log);
            var paymentSettings = paymentSystemBl.GetfnPartnerPaymentSettings(new FilterfnPartnerPaymentSetting(), true);
            return new ApiResponseBase
            {
