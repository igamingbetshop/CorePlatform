    ...
    public static class DashboardController
    {
                ...
                case Constants.RequestMethods.ExportClientsInfoList:
                    return
                        ExportClientsInfoList(
                            JsonConvert.DeserializeObject<ApiFilterfnClientDashboard>(request.RequestData), identity, log, env);
            }
        ...
        private static ApiResponseBase GetDeposits(ApiFilterDashboard input, SessionIdentity identity, ILog log)
        {
            using (var reportBl = new ReportBll(identity, log))
            {
                var response = new ApiResponseBase();
                var filter = input.MapToFilterDashboard();
                var responseList =  reportBl.GetDepositsForDashboard(filter);
                response.ResponseObject = responseList.Select(x => x.MapToApiDepositsInfo()).ToList();
                return response;
            }
        }
        ...
        private static ApiResponseBase GetWithdrawals(ApiFilterDashboard input, SessionIdentity identity, ILog log)
        {
            using (var reportBl = new ReportBll(identity, log))
            {
                var response = new ApiResponseBase();
                var filter = input.MapToFilterDashboard();
                var responseList = reportBl.GetWithdrawalsForDashboard(filter);
        ...
        private static ApiResponseBase ExportClientsInfoList(ApiFilterfnClientDashboard input, SessionIdentity identity, ILog log, IWebHostEnvironment env)
        {
            using (var reportBl = new ReportBll(identity, log))
            {
                ...
                string fileAbsPath = reportBl.ExportToCSV<ApiClientInfo>(fileName, clients, input.FromDate, input.ToDate, reportBl.GetUserIdentity().TimeZone, env);
