...
namespace IqSoft.CP.BLL.Interfaces
{
    public interface IClientBll : IBaseBll
    {
        ...
        VerifyCodeOutput VerifyClientEmail(string key, string mobileNumber, int? clientId, int partnerId, bool expire,
                                    List<Common.Models.WebSiteModels.SecurityQuestion> securityQuestions, int type, bool checkSecQuestions = true);
        ...
        List<ClientPaymentInfo> GetClientPaymentAccountDetails(int clientId, int? paymentSystemId, List<int> accountTypes, bool checkPermission, string walletNumber = null);

        List<AffiliateClientModel> GetClientsOfAffiliateManager(int managerId, int hours);

        List<fnTicket> OpenTickets(Ticket ticket, TicketMessage message, List<int> clientIds, List<int> segmentIds, bool checkPermissions);
        ...
        Document CreateCreditFromClient(ListOfOperationsFromApi transaction, DocumentBll documentBl, out LimitInfo info);
