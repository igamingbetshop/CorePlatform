...
namespace IqSoft.CP.BLL.Services
{
    public class ClientBll : PermissionBll, IClientBll
    {
        ...
        public Client RegisterClient(ClientRegistrationInput clientRegistrationInput, out List<int> userIds)
        {
                ...
                using (var notificationBl = new NotificationBll(regionBl))
                {
                    using (var bonusBl = new BonusService(regionBl))
                    {
                        ...
                        using (var scope = CommonFunctions.CreateTransactionScope())
                        {
                            ...
                            Db.SaveChanges();
                            userIds.AddRange(UploadRegistrationDocuments(dbClient.Id, clientRegistrationInput));
                            ...
                            if (!string.IsNullOrEmpty(clientRegistrationInput.ClientData.PinCode)) // check RegEx
                                SaveClientSetting(dbClient.Id, Constants.ClientSettings.PinCode, clientRegistrationInput.ClientData.PinCode, null, null);
                        ...
                        if (clientRegistrationInput.RegistrationType == (int)Constants.RegisterTypes.Full)
                        {
                        ...
                        userIds.AddRange(AddUserNotification(dbClient.PartnerId, (int)UserNotificationTypes.Registration, dbClient.Id, null, null, 0, null));
        ...
        public Client RegisterClient(Client client, out List<int> userIds)
        {
            userIds = new List<int>();
            ...
            Db.SaveChanges();
            userIds.AddRange(AddUserNotification(client.PartnerId, (int)UserNotificationTypes.Registration, client.Id, null, null, 0, null));
            return client;
        }

        public Client InsertClient(Client client, out List<int> userIds) //For IQ Wallet
        {
            var currentTime = DateTime.UtcNow;
            client.CreationTime = currentTime;
            userIds = new List<int>();
        ...
        private List<int> UploadRegistrationDocuments(int clientId, ClientRegistrationInput clientRegistrationInput)
        {
            ...
            if (clientRegistrationInput.IdCardDocumentByte != null)
            {
                ...
                }, string.Format("{0}.png", nameof(KYCDocumentTypes.IDCard)), clientRegistrationInput.IdCardDocumentByte, false, out List<int> userIds);
                resp.AddRange(userIds);
            }
            if (clientRegistrationInput.UtilityBillDocumentByte != null)
            {
                SaveKYCDocument(new ClientIdentity
                {
                    ...
                    Status = (int)KYCDocumentStates.InProcess
                }, string.Format("{0}.png", nameof(KYCDocumentTypes.UtilityBill)), clientRegistrationInput.UtilityBillDocumentByte, false, out List<int> userIds);
                resp.AddRange(userIds);
            }
            if (clientRegistrationInput.PassportDocumentByte != null)
            {
                SaveKYCDocument(new ClientIdentity
                {
                    ...
                    Status = (int)KYCDocumentStates.InProcess
                }, string.Format("{0}.png", nameof(KYCDocumentTypes.Passport)), clientRegistrationInput.PassportDocumentByte, false, out List<int> userIds);
                resp.AddRange(userIds);
            }
            if (clientRegistrationInput.DriverLicenseDocumentByte != null)
            {
                SaveKYCDocument(new ClientIdentity
                {
                    ...
                    Status = (int)KYCDocumentStates.InProcess
                }, string.Format("{0}.png", nameof(KYCDocumentTypes.DriverLicense)), clientRegistrationInput.DriverLicenseDocumentByte, false, out List<int> userIds);
                resp.AddRange(userIds);
            }
                                                 ...
                                                 bool checkSecQuestions = true)
        {
            ...
            if (clientId.HasValue)
            {
                ...
                clientInfo = Db.ClientInfoes.FirstOrDefault(x => x.Data == key && x.Type == type &&
                                                                 x.ObjectId == client.Id && x.ObjectTypeId == (int)ObjectTypes.Client);
                if (clientInfo == null)
        ...
        public VerifyCodeOutput VerifyClientEmail(string code, string email, int? clientId, int partnerId, bool expire,
                                                   List<Common.Models.WebSiteModels.SecurityQuestion> securityQuestions, 
                                                   int type, bool checkSecQuestions = true)
        {
            var response = new VerifyCodeOutput();
            DAL.ClientInfo clientInfo = null;
            if (clientId.HasValue)
            {
                ...
                {
                    clientInfo = Db.ClientInfoes.FirstOrDefault(x => x.Data == code && x.ObjectId == client.Id && x.ObjectTypeId == (int)ObjectTypes.Client &&
                    ...
                    clientInfo = Db.ClientInfoes.FirstOrDefault(x => x.ObjectId == client.Id && x.ObjectTypeId == (int)ObjectTypes.Client &&
                                                                     x.Data == code && x.Type == type);
            ...
            else
            {
                ...
                {
                    response.SecurityQuestions = Db.ClientSecurityAnswers.Where(x => x.ClientId == clientInfo.ObjectId).Select(x => x.SecurityQuestionId).ToList();
                    response.ClientId = clientInfo.ObjectId.Value;
                }
            }
            response.EmailOrMobile = clientInfo?.MobileOrEmail;
            return response;
        ...
        public ClientSession RefreshClientSession(string token, bool expireOld = false)
        {
            ...
            Db.SaveChanges();
            CacheManager.RemoveProductSessionsCount(session.ProductId);
            return savedSession;
        }
        ...
        public string ResetClientPinCode(int clientId)
        {
            GetPermissionsToObject(new CheckPermissionInput
            {
                Permission = Constants.Permissions.EditClientPinCode
            });
            ...
            if (string.IsNullOrEmpty(client.Email))
                throw BaseBll.CreateException(LanguageId, Constants.Errors.EmailCantBeEmpty);
            if (client.IsEmailVerified)
                throw BaseBll.CreateException(LanguageId, Constants.Errors.EmailNotVerified);
            var newPinCode = CommonFunctions.GetRandomString(10); // check RegEx
            SaveClientSetting(clientId, Constants.ClientSettings.PinCode, newPinCode, null, null);
            using (var notificationBll = new NotificationBll(Identity, Log))
            {
                var notificationModel = new NotificationModel
                {
                    ...
                    ObjectId = client.Id,
                    ObjectTypeId = (int)ObjectTypes.Client,
                    VerificationCode = newPinCode,
                    LanguageId = client.LanguageId,
                    ClientInfoType = (int)ClientInfoTypes.ResetPinCode,
                    MobileOrEmail = client.Email
                };
                notificationBll.SendNotificationMessage(notificationModel, out _);
            }
            return newPinCode;
        }

        public Client ChangeClientDataFromWebSite(ChangeClientFieldsInput clientData, NotificationBll notificationBl)
        {
            ...
            using (var transactionScope = CommonFunctions.CreateTransactionScope())
            {
                ...
                if (!string.IsNullOrEmpty(clientData.MobileNumber) && clientData.MobileNumber != dbClient.MobileNumber)
                {
                    ...
                    if (dbClient.IsMobileNumberVerified)
                    {
                        ...
                        if (!string.IsNullOrEmpty(restrictMobileNumberChanges) && restrictMobileNumberChanges == "1")
                            throw CreateException(LanguageId, Constants.Errors.NotAllowed);
                    }
        ...
        public Client ChangeClientDataFromBetShop(ChangeClientFieldsInput clientData)
        {
            ...
            using (var transactionScope = CommonFunctions.CreateTransactionScope())
            {
                ...
                if (!string.IsNullOrWhiteSpace(clientData.ZipCode.Trim()))
                    dbClient.ZipCode = clientData.ZipCode;
        ...
        public Client QuickRegisteration(QuickRegistrationInput quickRegistrationInput, out List<int> userIds)
        {
            bool sendPass = false;
            ...
            var clientRegInput = new ClientRegistrationInput
            {
                ...
                RegistrationType = quickRegistrationInput.IsMobile ? (int)RegisterTypes.Mobile : (int)RegisterTypes.Email,
            ...
            client = RegisterClient(clientRegInput, out userIds);
            using (var notificationBll = new NotificationBll(Identity, Log))
            {
                try
                {
        ...
        public int SendRecoveryToken(int partnerId, string languageId, string identifier, string recaptcha)
        {
            ...
            {
                if (isValidEmail)
                    return notificationBll.SendRecoveryEmail(client.PartnerId, client.Id, (int)ObjectTypes.Client, client.Email);
                return notificationBll.SendRecoverySMS(client.PartnerId, client.Id, (int)ObjectTypes.Client, client.MobileNumber);
            }
        }
            ...
            string languageId, List<Common.Models.WebSiteModels.SecurityQuestion> securityQuestions)
        {
            var clientInfo = Db.ClientInfoes.FirstOrDefault(x => x.Data == recoveryToken && x.PartnerId == partnerId &&
                                                                 x.ObjectTypeId == (int)ObjectTypes.Client) ??
                throw CreateException(languageId, Constants.Errors.WrongToken);
            if (clientInfo.State == (int)ClientInfoStates.Expired)
                throw CreateException(languageId, Constants.Errors.TokenExpired);
            var client = Db.Clients.First(x => x.Id == clientInfo.ObjectId);
        ...
        private void VerifyClientFieldsByWebSettings(Client client, int? registerType, bool idUsernameGenerated)
        {
            Dictionary<string, ItemProperty> webSettings;
            ...
            if (registerType.HasValue)
            {
                ...
                if (webSettings.ContainsKey("SMSCode") && !client.IsMobileNumberVerified)
                    throw CreateException(LanguageId, Constants.Errors.MobileNumberNotVerified);
            }
            else
                webSettings = Db.WebSiteSubMenuItems.Where(x => x.WebSiteMenuItem.WebSiteMenu.PartnerId == client.PartnerId &&
            ...
            {
                var value = field.GetValue(client, null);
                var p = webSettings[field.Name];
                if (!string.IsNullOrEmpty(p.Mandatory))
                {
                    if (!bool.TryParse(p.Mandatory.ToLower(), out bool isMandatory))
                        isMandatory = p.Mandatory != "0";
                    if (isMandatory)
                    {
                        if (value == null || string.IsNullOrEmpty(value.ToString()))
                            switch (field.Name)
                            {
                                case nameof(client.FirstName):
                                    throw CreateException(LanguageId, Constants.Errors.FirstNameCantBeEmpty);
                                case nameof(client.LastName):
                                    throw CreateException(LanguageId, Constants.Errors.LastNameCantBeEmpty);
                                case nameof(client.UserName):
                                    throw CreateException(LanguageId, Constants.Errors.UserNameCantBeEmpty);
                                case nameof(client.Email):
                                    if (registerType != (int)RegisterTypes.Mobile)
                                        throw CreateException(LanguageId, Constants.Errors.EmailCantBeEmpty);
                                    break;
                                case nameof(client.MobileNumber):
                                    if (registerType != (int)RegisterTypes.Email)
                                        throw CreateException(LanguageId, Constants.Errors.MobileNumberCantBeEmpty);
                                    break;
                                case nameof(client.Address):
                                    throw CreateException(LanguageId, Constants.Errors.AddressCantBeEmpty);
                                case nameof(client.ZipCode):
                                    throw CreateException(LanguageId, Constants.Errors.ZipCodeCantBeEmpty);
                                case nameof(client.CurrencyId):
        ...
        public List<AccountsBalanceHistoryElement> ExportClientAccountsBalanceHistory(FilterAccountsBalanceHistory filter, double timeZone)
        {
            return GetClientAccountsBalanceHistoryPaging(filter, timeZone);
        }
        public List<AccountsBalanceHistoryElement> GetClientAccountsBalanceHistoryPaging(FilterAccountsBalanceHistory filter, double timeZone)
        {
            ...
            foreach (var operation in operations)
            {
                if (operation.Amount > 0)
                {
                    ...
                    result.Add(new AccountsBalanceHistoryElement
                    {
                        ...
                        OperationTime = operation.CreationTime.GetGMTDateFromUTC(timeZone),
        ...
        public ClientIdentity SaveKYCDocument(ClientIdentity input, string documentName, byte[] imageData, bool checkPermission, out List<int> userIds)
        {
            userIds = new List<int>();
            var client = CacheManager.GetClientById(input.ClientId) ??
                throw CreateException(LanguageId, Constants.Errors.ClientNotFound);
            ...
            using (var transactionScope = CommonFunctions.CreateTransactionScope())
            {
                ...
                if (imageData != null)
                {
                    ...
                    var fileName = $"ClientDocuments/{Guid.NewGuid()}_{input.ClientId}_{documentName}";
            ...
            userIds.AddRange(AddUserNotification(client.PartnerId, (int)UserNotificationTypes.KYC, client.Id, null, null, 0, null));
                ...
                using (var documentBl = new DocumentBll(this))
                {
                    using (var transactionScope = CommonFunctions.CreateTransactionScope())
                    {
                        ...
                        if (client.UserId != oldClient.UserId)
                        {
                            if (client.UserId.HasValue)
                            {
                                ...
                                if (oldClient.PartnerId != user.PartnerId || oldClient.CurrencyId != user.CurrencyId ||
                                   (user.Type != (int)UserTypes.CompanyAgent && user.Type != (int)UserTypes.DownlineAgent))
                                    throw CreateException(LanguageId, Constants.Errors.NotAllowed);
                            }
                        ...
                        if (!string.IsNullOrEmpty(affInput.AffiliatePlatformId) && newRef == null)
                        {
                            ...
                            {
                                AffiliatePlatformId = Convert.ToInt32(affInput.AffiliatePlatformId),
        ...
        public void CheckDuplicates(int clientId, string ip)
        {
            var jobTrigger = new JobTrigger
            {
                ClientId = clientId,
                Info = ip,
                Type = (int)JobTriggerTypes.CheckDuplicates
            };
            Db.JobTriggers.AddIfNotExists(jobTrigger, x => x.ClientId == clientId &&
                                                           x.Info == ip &&
        ...
        public static BllClient LoginClient(LoginInput input, BllClient client, out string newToken, out RegionTree regionDetails, ILog log, bool checkPassword = true)
        {
            ...
            using (var clientBl = new ClientBll(new SessionIdentity { LanguageId = input.LanguageId }, log))
            {
                ...
                clientBl.CheckDuplicates(client.Id, input.Ip);
        ...
        public ProductInfo GetPartnerProductInfo(int? clientId, int productId, int partnerId)
        {
            ...
            var productInfo = new ProductInfo
            {
                ...
                RTP = partnerProductSetting.RTP ?? (product.RTP ?? 0),
                Volatility = string.IsNullOrWhiteSpace(volatility) ? product.Volatility : volatility,
                Tags = product.Lines
            };
            if (clientId.HasValue)
            {
                                                            ...
                                                            int? deviceType = null, string token = null, int? maxLenght = null)
        {
            ...
            CacheManager.RemoveProductSessionsCount(newSession.ProductId);
            return newSession;
        }
            ...
            using (var transactionScope = CommonFunctions.CreateTransactionScope())
            {
                using (var db = new IqSoftCorePlatformEntities())
                {
                    ...
                    if (sessionType != (int)SessionTypes.RefreshToken)
                    {
                        ...
                        db.ClientSessions.Where(x => x.ClientId == clientId && x.Type != (int)SessionTypes.RefreshToken &&
                            (x.State == (int)SessionStates.Pending || x.State == (int)SessionStates.Active))
        ...
        public void ReconsiderClientSegments(int clientId)
        {
            CheckPermission(Constants.Permissions.ReconsiderClientSegments);
            var checkClientPermission = GetPermissionsToObject(new CheckPermissionInput
            {
                Permission = Constants.Permissions.ViewClient,
                ObjectTypeId = (int)ObjectTypes.Client
            ...
            AddClientJobTrigger(clientId, (int)JobTriggerTypes.ReconsiderSegments);
        }
        ...
        private void VerifySmsCode(int clientId, string code)
        {
            ...
            {
                var clientInfo = Db.ClientInfoes.FirstOrDefault(x => x.ObjectId == client.Id && x.ObjectTypeId == (int)ObjectTypes.Client &&
                                                                     x.Data == code && x.Type == (int)ClientInfoTypes.AccountDetailsMobileKey);
                if (clientInfo == null)
                    throw CreateException(LanguageId, Constants.Errors.WrongVerificationKey);
                if (clientInfo.State == (int)ClientInfoStates.Expired)
                    throw CreateException(LanguageId, Constants.Errors.VerificationKeyExpired);
        ...
        public PagedModel<fnTicket> GetTickets(FilterTicket filter, bool checkPermissions)
        {
            ...
            Func<IQueryable<fnTicket>, IOrderedQueryable<fnTicket>> orderBy;
            if (filter.OrderBy.HasValue)
                orderBy = QueryableUtilsHelper.OrderByFunc<fnTicket>(filter.FieldNameToOrderBy, filter.OrderBy.Value);
            else
                orderBy = tickets => tickets.OrderByDescending(x => x.Id);

            return new PagedModel<fnTicket>
            {
        ...
        public List<int> AddUserNotification(int partnerId, int type, int? clientId, 
            long? paymentRequestId, int? bonusId, decimal amount, NotificationBll notificationBl)
        {
            var name = string.Empty;
            switch (type)
            {
                case (int)UserNotificationTypes.Registration:
                    name = UserConfigurations.RegistrationSubscription;
                    break;
                case (int)UserNotificationTypes.KYC:
                    name = UserConfigurations.KYCSubscription;
                    break;
                case (int)UserNotificationTypes.Deposit:
                    name = UserConfigurations.DepositSubscription;
                    break;
                case (int)UserNotificationTypes.Withdraw:
                    name = UserConfigurations.WithdrawSubscription;
                    break;
                case (int)UserNotificationTypes.Bonus:
                    name = UserConfigurations.BonusSubscription;
                    break;
            }
            var users = Db.UserConfigurations.Include(x => x.User).Where(x => x.Name == name && x.BooleanValue == true).ToList();
            foreach (var u in users)
            {
                var userPermissions = CacheManager.GetUserPermissions(u.UserId);
                var objects = CacheManager.GetUserAccessObjects(u.UserId);
                var partnerIds = objects.Where(x => x.ObjectTypeId == (int)ObjectTypes.Partner &&
                        x.PermissionId == Permissions.ViewPartner && Int32.TryParse(x.ObjectId, out int r)).Select(x => Convert.ToInt32(x.ObjectId)).ToList();
                if (userPermissions.Any(x => x.IsAdmin) || partnerIds.Contains(partnerId))
                {
                    var un = new UserNotification
                    {
                        ...
                        PaymentRequestId = paymentRequestId,
                        BonusId = bonusId,
                        Status = (int)NotificationStates.Pending,
                        CreationTime = DateTime.UtcNow
                    };
                    Db.UserNotifications.Add(un);
                }
                if(type == (int)UserNotificationTypes.Deposit && u.StringValue == "1" && (u.NumericValue == null || u.NumericValue.Value >= amount))
                {
                    notificationBl.SendNotificationMessage(new NotificationModel
                    {
                        PartnerId = partnerId,
                        ...
                        MobileOrEmail = u.User.Email,
                        ClientInfoType = (int)ClientInfoTypes.DepositEmail
                    }, out _);
                }
            }
            Db.SaveChanges();
            foreach (var u in users)
            {
                CacheManager.DeleteUserNotificationsCount(u.UserId);
            }
            return users.Select(x => x.UserId).ToList();
        }

        public void SaveClientPaymentSetting(int clientId, int partnerPaymentSettingId, int state)
        {
            var client = CacheManager.GetClientById(clientId) ??
                throw CreateException(LanguageId, Constants.Errors.ClientNotFound);
            var checkClientPermission = GetPermissionsToObject(new CheckPermissionInput
            ...
            if (state == (int)ClientPaymentStates.Active)
                Db.ClientPaymentSettings.Where(x => x.ClientId == clientId && x.PartnerPaymentSettingId == partnerPaymentSettingId).DeleteFromQuery();
            else
            {
                var currentTime = DateTime.UtcNow;
                var dbClientPaymentSetting = Db.ClientPaymentSettings.FirstOrDefault(x => x.ClientId == clientId &&
                                                                                          x.PartnerPaymentSettingId == partnerPaymentSettingId);
                if (dbClientPaymentSetting != null)
                {
                    dbClientPaymentSetting.State = state;
                    dbClientPaymentSetting.LastUpdateTime = currentTime;
                }
                else
                {
                    Db.ClientPaymentSettings.Add(new ClientPaymentSetting
                    {
                        ClientId = clientId,
            ...
            CacheManager.RemoveFromCache(string.Format("{0}_{1}", Constants.CacheItems.ClientPaymentSetting, clientId));
        }
        ...
        public List<ClientPaymentSetting> GetClientPaymentSettings(int clientId, int? state, bool checkPermission)
        {
            ...
            var partnerPaymentSettings = Db.PartnerPaymentSettings.Where(x => x.PartnerId == client.PartnerId &&
                                                                  ...
                                                                  .ToList();
            var clientPaymentSettings = Db.ClientPaymentSettings.Include(x => x.PartnerPaymentSetting.PaymentSystem)
        ...
        public void FairSegmentTriggers()
        {
            ...
            {
                var currentDate = DateTime.UtcNow;
                var triggerSegments = Db.TriggerSettings.Where(x => x.Status == (int)TriggerStatuses.Active && x.Type == (int)TriggerTypes.SegmentChange
                    && x.SegmentId.HasValue && x.StartTime <= currentDate && x.FinishTime >= currentDate).Select(x => x.SegmentId).Distinct().ToList();
        ...
        public List<ClientBonusTrigger> FairClientBonusTrigger(ClientTriggerInput triggerToFair, out bool alreadyAdded)
        {
            var result = new List<ClientBonusTrigger>();
            alreadyAdded = false;
            var currentTime = GetServerDate();
            var client = CacheManager.GetClientById(triggerToFair.ClientId);
            var partner = CacheManager.GetPartnerById(client.PartnerId);

            foreach (var cb in triggerToFair.ClientBonuses)
            {
                ...
                foreach (var g in bonus.TriggerGroups)
                {
                    foreach (var tgs in g.TriggerGroupSettings)
                    {
                        ...
                        if (CheckTriggerAcceptance(triggerToFair, trigger))
                        {
                            var existingTrigger = Db.ClientBonusTriggers.FirstOrDefault(x => x.ClientId == triggerToFair.ClientId &&
                                ...
                                (existingTrigger.WageringAmount != null && existingTrigger.WageringAmount < triggerMinAmount)) //wageringamount works with minamount
                            {
                                    ...
                                    trigger.Type == (int)TriggerTypes.BetPlacementAndSettlement) && !string.IsNullOrEmpty(trigger.Condition))
                                {
                                    ...
                                    var info = string.IsNullOrEmpty(triggerToFair.TicketInfo) ? new BonusTicketInfo() : 
                                        JsonConvert.DeserializeObject<BonusTicketInfo>(triggerToFair.TicketInfo);
                                    info.BetAmount = triggerToFair.SourceAmount != null ? triggerToFair.SourceAmount.Value : (triggerMinAmount ?? 0);
                                    if (!string.IsNullOrEmpty(triggerToFair.WinInfo))
                                    {
                                        var win = JsonConvert.DeserializeObject<BonusTicketInfo>(triggerToFair.WinInfo);
                                        ...
                                        if (trigger.BonusSettingCodes == "2")
                                            triggerToFair.SourceAmount = win.WinAmount - info.BetAmount;
                                    }
                                    accept = IsCorrectConditionGroup(conditions, info, client.CurrencyId, partner.CurrencyId);
                                    Log.Info("FairClientBonusTrigger_" + triggerToFair.ClientId + trigger.Condition + "_" + JsonConvert.SerializeObject(info) + "_" +
                                        JsonConvert.SerializeObject(trigger) + "_" + accept);
                                }
                                else if (trigger.Type == (int)TriggerTypes.CrossProductBetPlacement && triggerToFair.ProductId != null)
                                {
                                    while (true)
                                    {
                                        var pr = trigger.ProductSettings.FirstOrDefault(x => x.ProductId == triggerToFair.ProductId.Value);
                                        if (pr != null)
                                        {
                                            accept = (pr.Percent > 0);
                                            percent = pr.Percent;
                                        ...
                                        {
                                            var product = CacheManager.GetProductById(triggerToFair.ProductId.Value);
                                            if (!product.ParentId.HasValue)
                                                break;
                                            triggerToFair.ProductId = product.ParentId.Value;
                                        }
                                    }
                                }
                                ...
                                if (accept)
                                {
                                    ...
                                    if (isBetTrigger || triggerToFair.SourceAmount == null || (
                                        (triggerMinAmount == null || triggerToFair.SourceAmount.Value >= triggerMinAmount.Value) &&
                                        (triggerMaxAmount == null || triggerToFair.SourceAmount.Value <= triggerMaxAmount.Value))
                                        )
                                    {
                                        if (existingTrigger == null)
                                        {
                                            existingTrigger = new ClientBonusTrigger
                                            {
                                                ClientId = triggerToFair.ClientId,
                                                TriggerId = trigger.Id,
                                                BonusId = cb.BonusId,
                                                SourceAmount = (trigger.Type == (int)TriggerTypes.DailyDeposit ? triggerUpToAmount : amount),
                                                CreationTime = DateTime.UtcNow,
                                                BetCount = (isBetTrigger || trigger.Type == (int)TriggerTypes.DailyDeposit) ? 1 : (int?)null,
                                                WageringAmount = isBetTrigger && triggerMinAmount != null ? (triggerToFair.SourceAmount * (percent ?? 100)) / 100 :
                                                    (trigger.Type == (int)TriggerTypes.DailyDeposit ? Math.Floor((triggerMinAmount ?? 0) * 100) / 100m : (decimal?)null),
                                            ...
                                            if (isBetTrigger && triggerMaxAmount.HasValue && triggerMaxAmount.Value < amount)
                                                existingTrigger.SourceAmount = triggerMaxAmount;

                                            Db.ClientBonusTriggers.Add(existingTrigger);
                                        }
                                        else
                                        {
                                            if (trigger.Type == (int)TriggerTypes.DailyDeposit)
                                            {
                                                var sDate = existingTrigger.LastActionDate.Value.Date;
                                                var tDate = currentTime.Date;

                                                if ((tDate - sDate).TotalDays == 1)
                                                {
                                                    existingTrigger.BetCount = existingTrigger.BetCount.Value + 1;
                                                    existingTrigger.LastActionDate = currentTime;
                                                }
                                                else if ((tDate - sDate).TotalDays > 1)
                                                {
                                                    existingTrigger.BetCount = 1;
                                                    existingTrigger.LastActionDate = currentTime;
                                                }
                                            }
                                            else
                                            {
                                                if (existingTrigger.BetCount != null)
                                                    existingTrigger.BetCount = existingTrigger.BetCount.Value + 1;
                                                if (existingTrigger.WageringAmount != null)
                                                    existingTrigger.WageringAmount = existingTrigger.WageringAmount.Value + (triggerToFair.SourceAmount * (percent ?? 100)) / 100;
                                                if (isBetTrigger)
                                                {
                                                    existingTrigger.SourceAmount += amount;
                                                    if (triggerMaxAmount != null && existingTrigger.SourceAmount >= triggerMaxAmount)
                                                        existingTrigger.SourceAmount = triggerMaxAmount;
                                                }
                                                existingTrigger.LastActionDate = currentTime;
                                            }
        ...
        private bool CheckTriggerAcceptance(ClientTriggerInput triggerToFair, TriggerSettingInfo trigger)
        {
            var currentTime = DateTime.UtcNow;
            if (trigger.StartTime > currentTime || trigger.FinishTime <= currentTime)
                return false;
            ...
            if (trigger.Type == triggerToFair.TriggerType)
            {
                if (triggerToFair.TriggerType == (int)TriggerTypes.PromotionalCode && trigger.BonusSettingCodes != triggerToFair.PromoCode)
                    return false;
                if (triggerToFair.TriggerType == (int)TriggerTypes.NthDeposit && trigger.Condition != triggerToFair.DepositsCount.ToString())
                    return false;
                if (triggerToFair.TriggerType == (int)TriggerTypes.SegmentChange && trigger.SegmentId != triggerToFair.SegmentId)
                    return false;
            ...
            {
                if (triggerToFair.TriggerType == (int)TriggerTypes.BetSettlement && trigger.Type == (int)TriggerTypes.BetPlacementAndSettlement)
                    return true;
                if (triggerToFair.TriggerType == (int)TriggerTypes.NthDeposit && (trigger.Type == (int)TriggerTypes.AnyDeposit || trigger.Type == (int)TriggerTypes.DailyDeposit))
                    return true;
                if (triggerToFair.TriggerType == (int)TriggerTypes.SignUp && trigger.Type == (int)TriggerTypes.SignupCode && trigger.BonusSettingCodes == triggerToFair.PromoCode)
                    return true;
        ...
        public ClientBonusTrigger ChangeClientBonusTriggerManually(int clientId, int triggerId, int bonusId, long reuseNumber, decimal? sourceAmount, int status)
        {
            ...
            CheckPermission(Constants.Permissions.ViewBonuses);
            var partnerAccess = GetPermissionsToObject(new CheckPermissionInput
            {
                Permission = Constants.Permissions.ViewPartner,
                ObjectTypeId = (int)ObjectTypes.Partner
        ...
        public List<TriggerSettingItem> GetClientBonusTriggers(int clientId, int bonusId, long reuseNumber, bool checkPermission)
        {
            var client = CacheManager.GetClientById(clientId) ??
                throw CreateException(LanguageId, Constants.Errors.ClientNotFound);
            if (checkPermission)
            ...
            var partner = CacheManager.GetPartnerById(client.PartnerId);
            var settings = Db.TriggerGroups.Where(x => x.BonusId == bonusId)
                                           .Select(x => new
                                           {
                                               Id = x.Id,
                                               Name = x.Name,
            ...
            var triggerSettings = new List<TriggerSettingItem>();
            ...
            foreach (var s in settings)
            {
                ...
                triggerSettings.AddRange(s.TriggerSettings);
            }

            var clientBonusTriggers = Db.ClientBonusTriggers.Where(x => x.BonusId == bonusId && (x.ClientId == clientId || x.ClientId == null) &&
                                                                        x.ReuseNumber == reuseNumber).ToList();
            var specialTriggers = new List<int>
            {
            ...
            foreach (var ts in triggerSettings)
            {
                ts.MinAmount = ts.MinAmount.HasValue ? Math.Floor(ConvertCurrency(partner.CurrencyId, client.CurrencyId, ts.MinAmount.Value) * 100) / 100m : (decimal?)null;
                ts.MaxAmount = ts.MaxAmount.HasValue ? Math.Floor(ConvertCurrency(partner.CurrencyId, client.CurrencyId, ts.MaxAmount.Value) * 100) / 100m : (decimal?)null;
                var clientBonusTrigger = clientBonusTriggers.FirstOrDefault(x => x.TriggerId == ts.Id);
                if (clientBonusTrigger != null)
                {
                ...
                else
                {
                    ...
                    if (specialTriggers.Contains(ts.Type))
                    {
                        ts.BetCount = 0;
                        ts.WageringAmount = ts.MinAmount.HasValue ? 0 : (decimal?)null;
                    }
                }
            }
        ...
        */
            ...
            using (var scope = CommonFunctions.CreateTransactionScope())
            {
                ...
                if (dailyLimit?.NumericValue != null || maxPendingWithdrawalsCount?.NumericValue != null)
                {
                    ...
                    if (dailyLimit?.NumericValue != null)
                    {
                        ...
                        if (query.Count() >= dailyLimit.NumericValue)
                            throw CreateException(LanguageId, Constants.Errors.ClientDailyLimitExceeded);
                    }
                    if (maxPendingWithdrawalsCount?.NumericValue != null)
                    {
                        query = query.Where(x => x.Status != (int)PaymentRequestStates.CanceledByClient &&
                ...
                if ((!partnerPaymentSetting.ApplyPercentAmount.HasValue || request.Amount >= partnerPaymentSetting.ApplyPercentAmount) &&
                    (request.Amount - partnerPaymentSetting.FixedFee - request.Amount * partnerPaymentSetting.Commission / 100m) <= 0)
                    throw CreateException(LanguageId, Constants.Errors.WrongOperationAmount);
        ...
        public List<int> SplitWithdrawalRequest(long paymentRequestId, List<decimal> installments)
        {
            ...
            using (var notificationBl = new NotificationBll(this))
            {
                using (var scope = CommonFunctions.CreateTransactionScope())
                {
                    ChangeWithdrawRequestState(paymentRequest.Id, PaymentRequestStates.Splitted, "Split", paymentRequest.CashDeskId,
                                               null, false, paymentRequest.Parameters, documentBl, notificationBl, out response);
                    var currentTime = DateTime.UtcNow;
        ...
        public ChangeWithdrawRequestStateOutput ChangeWithdrawRequestState(long requestId, PaymentRequestStates state, string comment,
                    int? cashDeskId, int? cashierId, bool checkPermission, string parameters, DocumentBll documentBl, 
                    NotificationBll notificationBl, out List<int> userIds, bool sendEmail = false, bool changeFromPaymentSystem = false)
        {
            ...
            {
                var partner = CacheManager.GetPartnerById(client.PartnerId);
                userIds.AddRange(AddUserNotification(client.PartnerId, (int)UserNotificationTypes.Withdraw, null,
                    requestId, null, ConvertCurrency(client.CurrencyId, partner.CurrencyId, response.RequestAmount), null));
            }
        ...
        public Document PayWithdrawFromPaymentSystem(ChangeWithdrawRequestStateOutput resp, DocumentBll documentBl, NotificationBll notificationBl, MerchantRequest mr = null)
        {
            ...
            var isAffiliate = client.AffiliateReferralId.HasValue;
            if (isAffiliate || 
                !string.IsNullOrEmpty(CacheManager.GetPartnerSettingByKey(client.PartnerId, Constants.PartnerKeys.CRMPlarforms).StringValue))
            {
                var currentDate = DateTime.UtcNow;
                AffiliateReferral clientAffiliate = null;

                if (isAffiliate)
                  clientAffiliate = Db.AffiliateReferrals.Include(x => x.AffiliatePlatform).FirstOrDefault(x => x.Id == client.AffiliateReferralId);
        ...
        public PaymentRequest UploadPaymentForm(PaymentRequest paymentRequest, string paymentForm, string imageName, out List<int> userIds)
        {
            userIds = new List<int>();
            var client = CacheManager.GetClientById(paymentRequest.ClientId.Value);
            ...
            if (paymentRequest.Type == (int)PaymentRequestTypes.Deposit)
            {
                if (!string.IsNullOrEmpty(paymentForm))
                {
                    ...
                    var imgName = $"ClientPaymentForms/{Guid.NewGuid()}_{client.Id}_{imageName}";
                    var path = $"ftp://{ftpModel.Url}/{imgName}";
                    UploadFtpImage(Convert.FromBase64String(paymentForm), ftpModel, path);
            ...
            else
            {
                using (var documentBl = new DocumentBll(this))
                {
                    using (var notificationBl = new NotificationBll(this))
                    {
                        ...
                        {
                            ChangeWithdrawRequestState(request.Id, PaymentRequestStates.Confirmed, "", request.CashDeskId, null, false, string.Empty, documentBl, notificationBl, out userIds);
                        }
                        return request;
                    }
                }
        ...
        public PaymentRequest CreateManualDeposit(PaymentRequestInput paymentRequestInput)
        {
            ...
            var partnerPaymentSetting = CacheManager.GetPartnerPaymentSettings(client.PartnerId, paymentRequestInput.PaymentSystemId, amca ? paymentRequestInput.CurrencyId : client.CurrencyId,
            ...
            var paymentRequest = new PaymentRequest
            {
                ...
                CommissionAmount = commissionAmount,
                CurrencyId = amca ? paymentRequestInput.CurrencyId : client.CurrencyId,
                Status = (int)PaymentRequestStates.Pending,
                PartnerPaymentSettingId = partnerPaymentSetting.Id,
        ...
        public void ApproveDepositFromPaymentSystem(PaymentRequest request, bool fromAdmin, out List<int> userIds, string comment = "",
            ClientPaymentInfo info = null, MerchantRequest mr = null, long? accountId = null)
        {
            ...
            using (var documentBl = new DocumentBll(this))
            {
                using (var notificationBl = new NotificationBll(this))
                {
                    ...
                    if ((!partnerPaymentSetting.ApplyPercentAmount.HasValue || request.Amount >= partnerPaymentSetting.ApplyPercentAmount) &&
                        partnerPaymentSetting.Commission > 0)
                    ...
                    if (!Db.Documents.Any(x => x.OperationTypeId == (int)OperationTypes.Deposit &&
                                               x.PartnerPaymentSettingId == partnerPaymentSetting.Id &&
                                               x.ExternalTransactionId == paymentRequest.ExternalTransactionId))
                    {
                        var operation = new Operation
                        {
                            Amount = request.Amount,
                    ...
                    var partner = CacheManager.GetPartnerById(client.PartnerId);
                    userIds.AddRange(AddUserNotification(client.PartnerId, (int)UserNotificationTypes.Deposit, null, 
                        paymentRequest.Id, null, ConvertCurrency(client.CurrencyId, partner.CurrencyId, pureAmount), notificationBl));
                    CacheManager.RemoveClientBalance(client.Id);
        ...
        public bool GiveAffiliateCommission(BllClient client, long paymetRequestId, int partnerPaymentSettingId,
            int depCount, decimal depositAmount, DocumentBll documentBl)
        {
            var result = false;
            if (client.AffiliateReferralId != null)
            {
                    ...
                    ar.Type == (int)AffiliateReferralTypes.InternalAffiliatePlatform && Int32.TryParse(ar.AffiliateId, out int affId))
                {
                    ...
                    if (commission != null && (commission.DepositCount == null || depCount <= commission.DepositCount.Value))
                    {
        ...
        public Document CreateCreditFromClient(ListOfOperationsFromApi transaction, DocumentBll documentBl, out LimitInfo info)
        {
            ...
            if (clientState == (int)ClientStates.FullBlocked || clientState == (int)ClientStates.Disabled ||
                ...
                clientState == (int)ClientStates.SuspendedWithWithdraw)
                throw CreateException(LanguageId, Constants.Errors.ClientBlocked);
            ...
            using (var scope = CommonFunctions.CreateTransactionScope())
            {
                ...
                if (operationTypeId == (int)OperationTypes.Bet)
                {
                    ...
                    var cmm = CacheManager.GetPartnerSettingByKey(operationItemFromProduct.Client.PartnerId, Constants.PartnerKeys.BetCommissionPercent);
                    if (cmm != null && cmm.Id > 0 && cmm.NumericValue != null && cmm.NumericValue > 0)
                    {
                        operation.Amount += operation.Amount * cmm.NumericValue.Value / 100;
                    }
                ...
                if (externalPlatformType == null || externalPlatformType.NumericValue == null ||
                    externalPlatformType.NumericValue.Value != (int)PartnerTypes.ExternalPlatform ||
                    !operationItemFromProduct.Client.UserName.Contains(Constants.ExternalClientPrefix))
                {
                    var item = new OperationItem
                    {
                        ...
                        Amount = transaction.BonusId > 0 ? 0 : operation.Amount,
                        CurrencyId = transaction.CurrencyId,
                        Type = (int)TransactionTypes.Debit,
                ...
                if (operationTypeId == (int)OperationTypes.Bet)
                {
                    if (transaction.BonusId > 0)
                    {
                        UpdateClientFreeBetBonus(transaction.BonusId, operationItemFromProduct.Client.Id, initialOperationAmount);
                    ...
                    else
                    {
                        ...
                        FairClientBonusTrigger(new ClientTriggerInput
                        {
                            ...
                            SourceAmount = initialOperationAmount,
                            BonusId = operation.BonusId
                        }, out bool alreadyAdded);
                    }
                }
                Db.SaveChanges();
        ...
        public Document CreateDebitToClient(ClientOperation transaction, int clientId, string userName, DocumentBll documentBl, Document creditTransaction)
        {
            ...
            {
                if (transaction.ProductId == Constants.SportsbookProductId || Constants.ExternalSportsbookProductIds.Contains(transaction.ProductId ?? 0))
                {
                ...
                else if (transaction.ExternalTransactionId.StartsWith(Constants.FreeSpinPrefix))
                {
                    var clientBonuses = CacheManager.GetClientNotAwardedCampaigns(clientId);
                    ...
                    FairClientBonusTrigger(new ClientTriggerInput
                    {
                        ClientId = clientId,
                        ClientBonuses = clientBonuses,
                        TriggerType = (int)TriggerTypes.FreeSpinWin,
                        SourceAmount = transaction?.Amount ?? 0m,
                        TicketInfo = creditTransaction == null ? string.Empty : creditTransaction.TicketInfo,
                        WinInfo = transaction.Info,
                        ProductId = transaction.ProductId
                    }, out _);
        ...
        public List<int> ChangeWithdrawPaymentRequestState(long requestId, string comment, int? cashDeskId, int? cashierId, PaymentRequestStates state)
        {
            var resp = new List<int>();
            switch (state)
            {
            ...
            using (var documentBl = new DocumentBll(this))
            {
                ...
                {
                    ChangeWithdrawRequestState(requestId, state, comment, cashDeskId, cashierId, true, string.Empty, documentBl, notificationBl, out resp);
                }
            }
            return resp;
        }
        ...
        public ApiAdminClientSetting GetClientSettings(int clientId, bool checkPermission)
        {
            ...
            if (!string.IsNullOrEmpty(underMonitoring?.StringValue))
            {
                ...
                apiClientSetting.PEPSanctioned = new StatusModel { Status = underMonitoringTypes.Contains((int)UnderMonitoringTypes.PEPSanction).ToString(), CheckedAt = underMonitoring?.DateValue };
                apiClientSetting.UnderHighRiskCountry = new StatusModel { Status = underMonitoringTypes.Contains((int)UnderMonitoringTypes.HighRiskCountry).ToString(), CheckedAt = underMonitoring?.LastUpdateTime };
            }
            else
            {
                apiClientSetting.PEPSanctioned = new StatusModel { Status = false.ToString(), CheckedAt = client.CreationTime };
                apiClientSetting.UnderHighRiskCountry = new StatusModel { Status = false.ToString(), CheckedAt = client.CreationTime };
        ...
        public ApiClientLimit GetClientLimitSettings(int clientId, bool checkPermission, bool isHistoryChange = false)
        {
            ...
            foreach (var setting in dbSettings)
            {
                ...
                if (limit != null && limit.PropertyType.Name == nameof(LimitItem) && setting.NumericValue.HasValue)
                {
            ...
            var st = CacheManager.GetClientSettingByName(clientId, ClientSettings.SelfExcluded);
            if (st != null)
            {
                apiClientLimit.SelfExcluded = new SelfExcludedModel
                {
                    ExcludedUntil = st.DateValue?.GetGMTDateFromUTC(Identity.TimeZone)
                };
                if (int.TryParse(st.StringValue, out int reason))
                {
                    apiClientLimit.SelfExcluded.Reason = reason;
                    apiClientLimit.SelfExcluded.ReasonText = CacheManager.GetPartnerCommentTemplates(client.PartnerId, (int)CommentTemplateTypes.ExclusionReason, LanguageId)
                                                              .FirstOrDefault(x => x.Id == reason)?.Text;
                }
            }
            st = CacheManager.GetClientSettingByName(clientId, ClientSettings.SystemExcluded);
            if (st != null)
            {
                apiClientLimit.SystemExcluded = new SelfExcludedModel
                {
                    ExcludedUntil = st.DateValue?.GetGMTDateFromUTC(Identity.TimeZone)
                };
                if (int.TryParse(st.StringValue, out int reason))
        ...
        public void RemoveClientExclusion(int clientId, bool isSystem, string comment)
        {
            ...
            SaveChangesWithHistory((int)ObjectTypes.ClientLimit, clientId, JsonConvert.SerializeObject(oldLimitSettings), comment);
            if (isSystem)
        ...
        {
            var currentTime = DateTime.UtcNow;
            var existingAccounts = Db.Accounts.Where(x => x.ObjectTypeId == (int)ObjectTypes.Client && x.ObjectId == clientId).ToList();
            var accounts = new List<Account>();
        ...
        public ClientSetting StakeNFT(ApiNFTInfo input)
        {
            ...
            if (t == null)
            {
                sValue.Add(new NFTInfo
                {
                    ContractAddress = input.ContractAddress,
                    Id = input.Id,
                    StakedUntil = input.StakedUntil
                });
            }
            else
                t.StakedUntil = input.StakedUntil;
            clientSetting.StringValue = JsonConvert.SerializeObject(sValue);
            SaveChangesWithHistory((int)ObjectTypes.ClientSetting, client.Id, oldSettings, string.Empty);
            return clientSetting;
        }

        public ClientSetting PledgeNFT(ApiNFTInfo input)
        {
            var client = CacheManager.GetClientById(input.ClientId) ??
                throw CreateException(LanguageId, Constants.Errors.ClientNotFound);
            var clientAccess = GetPermissionsToObject(new CheckPermissionInput
            ...
            if ((!clientAccess.HaveAccessForAllObjects && clientAccess.AccessibleObjects.All(x => x != input.ClientId)) ||
                (!partnerAccess.HaveAccessForAllObjects && partnerAccess.AccessibleIntegerObjects.All(x => x != client.PartnerId)))
                throw CreateException(LanguageId, Constants.Errors.DontHavePermission);
            if (!Enum.IsDefined(typeof(NFTFamilies), input.Family))
                throw CreateException(LanguageId, Constants.Errors.WrongInputParameters);
            ...
            var sValue = string.IsNullOrEmpty(clientSetting.StringValue) ? new List<NFTInfo>() : JsonConvert.DeserializeObject<List<NFTInfo>>(clientSetting.StringValue);
            var t = sValue.FirstOrDefault(x => x.Id == input.Id);
            if (t == null || sValue.Any(x => x.Family != null))
                throw CreateException(LanguageId, Constants.Errors.WrongInputParameters);
            t.Family = input.Family;
            t.PledgedUntil = input.PledgedUntil;
        ...
        public ApiClientStatistics GetClientStatisticsInfo(int clientId)
        {
            using (var dwh = new IqSoftDataWarehouseEntities())
            {
                var result = dwh.Gtd_Client_Info.Where(x => x.ClientId == clientId).GroupBy(x => 1).Select(x => new ApiClientStatistics
                {
                    TotalBetAmount = x.Sum(y => y.TotalBetAmount),
