namespace IqSoft.CP.BetShopWebApi.Common
{
	public class ApiMethods
	{
		public const string Authorization = "Authorization";
		public const string Login = "Login";
		public const string CardReaderAuthorization = "CardReaderAuthorization";
		public const string GetCashierSessionByToken = "GetCashierSessionByToken";
		public const string GetCashierSessionByProductId = "GetCashierSessionByProductId";
		public const string GetProductSession = "GetProductSession";
		public const string PlaceBet = "PlaceBet";
		public const string GetBookedBet = "GetBookedBet";
		public const string DoBet = "DoBet";
		public const string GetCashierInfo = "GetCashierInfo";
		public const string CloseSession = "CloseSession";
		public const string GetClient = "GetClient";
		public const string EditClient = "EditClient";
		public const string ResetClientPassword = "ResetClientPassword";
		public const string GetClients = "GetClients";
		public const string GetCashDesks = "GetCashDesks";
		public const string RegisterClient = "RegisterClient";
		public const string DepositToInternetClient = "DepositToInternetClient";
		public const string RollbackDepositToInternetClient = "RollbackDepositToInternetClient";
		public const string GetPaymentRequests = "GetPaymentRequests";
		public const string PayPaymentRequest = "PayPaymentRequest";
		public const string GetTicketInfo = "GetTicketInfo";
		public const string GetBetShopBets = "GetBetShopBets";
		public const string PayWin = "PayWin";
		public const string GetBetByBarcode = "GetBetByBarcode";
		public const string SendBetSelections = "SendBetSelections";
		public const string CancelBetSelection = "CancelBetSelection";
        public const string GetUnitResults = "unitresults";
        public const string GetUnitInfo = "unitresult";
        public const string GetCashDeskInfo = "GetCashDeskInfo";
		public const string GetChatMessages = "GetChatMessages";
		public const string SaveChatMessage = "SaveChatMessage";
		public const string ChatMessage = "ChatMessage";
		public const string GetBetInfo = "GetBetInfo";
		public const string GetShiftReport = "GetShiftReport";
		public const string CreateDebitCorrectionOnCashDesk = "CreateDebitCorrectionOnCashDesk";
		public const string CreateCreditCorrectionOnCashDesk = "CreateCreditCorrectionOnCashDesk";
		public const string GetCashDeskOperations = "GetCashDeskOperations";
		public const string GetCashDesksBalancesByDate = "GetCashDesksBalancesByDate";
		public const string GetCashDeskCurrentBalance = "GetCashDeskCurrentBalance";
		public const string CloseShift = "CloseShift";
		public const string GetAvailableProducts = "GetAvailableProducts";
		public const string GetBetShopOperations = "GetBetShopOperations";
		public const string LogoutUser = "LogoutUser";
		public const string GetBalance = "GetBalance";
		public const string PlaceBetByBarcode = "PlaceBetByBarcode";
		public const string GetBetByDocumentId = "GetBetByDocumentId";
		public const string GetUserProductSessions = "GetUserProductSessions";
        public const string GetResultsReport = "GetResultsReport";
        public const string GetUnitResult = "GetUnitResult";
        public const string ChangeCashierPassword = "ChangeCashierPassword";
		public const string WithdrawTerminalFunds = "WithdrawTerminalFunds";
		public const string DepositToTerminal = "DepositToTerminal";
		public const string Cashout = "Cashout";
		public const string GetRegions = "GetRegions";
		public const string CreateWithdrawPaymentRequest = "CreateWithdrawPaymentRequest";
		public const string GetErrorById = "GetErrorById";
		public const string AssignPin = "AssignPin";
	}
}