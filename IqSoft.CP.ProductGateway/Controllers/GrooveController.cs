...
{
    public class GrooveController : ApiController
    {
        private static readonly int ProviderId = CacheManager.GetGameProviderByName(Constants.GameProviders.GrooveGaming).Id;
        public static List<string> WhitelistedIps = CacheManager.GetProviderWhitelistedIps(Constants.GameProviders.GrooveGaming);
        ...
        public HttpResponseMessage ApiRequest([FromUri] BaseInput input)
        {
            ...
            catch (FaultException<BllFnErrorType> fex)
            {
                ...
                if (output.Code == GrooveHelpers.StatusCodes.Success)
                {
                    ...
                    if (isExternalPlatformClient)
                        ...
                        balance = BaseHelpers.GetClientProductBalance(clientId, productId);
            ...
            catch (Exception ex)
            {
                ...
                output.Message = GrooveHelpers.GetErrorMessage(output.Code);
                var bodyStream = new StreamReader(HttpContext.Current.Request.InputStream);
                WebApiApplication.DbLogger.Error($"Error: {ex} InputString: {bodyStream.ReadToEnd()} Response: {JsonConvert.SerializeObject(output)}");
            }
                        ...
                        else
                        {
                            ...
                            if (isExternalPlatformClient)
                            {
                                try
                                {
                                    var epBalance = ExternalPlatformHelpers.CreditFromClient(Convert.ToInt32(externalPlatformType.StringValue), client,
                                                                                             clientSession.ParentId ?? 0,
                                                                                             betOperationsFromProduct, betDocument, WebApiApplication.DbLogger);
                                    BaseHelpers.BroadcastBalance(client.Id, epBalance);
                                }
                                catch (Exception ex)
                                {
