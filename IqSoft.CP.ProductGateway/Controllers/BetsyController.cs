...
{
    public class BetsyController : ApiController
    {
        ...
        public HttpResponseMessage Authentication([FromUri] int partnerId)
        {
            ...
            try
            {
                ...
                outputString = JsonConvert.SerializeObject(response);
            }
            catch (FaultException<BllFnErrorType> fex)
            {
                ...
                outputString = JsonConvert.SerializeObject(errorOutput);
            ...
            var resp =  new HttpResponseMessage
            {
                ...
                Content = new StringContent(outputString),
            };
            resp.Content.Headers.Add("x-sign-jws", GenerateJWTToken(partnerId, outputString));
            return resp;
        }
        ...
        public HttpResponseMessage GetBalance([FromUri] int partnerId)
        {
            ...
            try
            {
                ...
                var response = new
                {
                    userId = client.Id.ToString(),
                    currency = client.CurrencyId,
                    amount = Convert.ToDecimal(string.Format("{0:N2}", BaseHelpers.GetClientProductBalance(client.Id, 0)))
                };
                outputString = JsonConvert.SerializeObject(response);
            }
            catch (FaultException<BllFnErrorType> fex)
        ...
        public HttpResponseMessage CheckPayment([FromUri]int partnerId)
        {
            var inputString = string.Empty;
            string outputString;
            var httpStatusCode = HttpStatusCode.OK;
        ...
        public HttpResponseMessage DoBetWin([FromUri] int partnerId)
        {
            var inputString = string.Empty;
            string outputString;
            var httpStatusCode = HttpStatusCode.OK;
