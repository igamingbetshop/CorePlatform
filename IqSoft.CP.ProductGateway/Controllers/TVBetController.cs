...
namespace IqSoft.CP.ProductGateway.Controllers
{
    public class TVBetController : ApiController
    {
        ...
        public static List<string> WhitelistedIps = CacheManager.GetProviderWhitelistedIps(Constants.GameProviders.TVBet);
        [HttpPost]
        [Route("{partnerId}/api/TVBet/GetUserData")]
        public HttpResponseMessage GetUserData(int partnerId, BaseInput input)
        {
        ...
        public HttpResponseMessage RefreshToken(int partnerId, BaseInput input)
        {
            var signature = input.Signature;
            input.Signature = null;
            var secureKey = string.Empty;
        ...
        public HttpResponseMessage MakePayment(int partnerId, TransactionInput input)
        {
            var signature = input.Signature;
            input.Signature = null;
            var secureKey = string.Empty;
            ...
            catch (FaultException<BllFnErrorType> fex)
            {
                ...
                WebApiApplication.DbLogger.Error( "Input:" + JsonConvert.SerializeObject(input) + "  Output: " + JsonConvert.SerializeObject(response));
            }
            catch (Exception ex)
            {
                response.IsSuccess = false;
                ...
                response.ErrorDescription = ex.Message;
                WebApiApplication.DbLogger.Error(ex);
                WebApiApplication.DbLogger.Error("Input:" + JsonConvert.SerializeObject(input) + "  Output: " + ex);
        ...
        private BaseModel DoBet(TransactionInput input)
        {
            var clientSession = ClientBll.GetClientProductSession(input.Token, Constants.DefaultLanguageId);
        ...
        private BaseModel DoWin(TransactionInput input)
        {
        ...
        private BaseModel Refund(TransactionInput input)
        {
            var clientSession = ClientBll.GetClientProductSession(input.Token, Constants.DefaultLanguageId, null, false);
            using (var documentBl = new DocumentBll(new SessionIdentity(), WebApiApplication.DbLogger))
            {
                ...
                if (document == null || document.State == (int)BetDocumentStates.Deleted)
                    return new BaseModel
                    {
                        ...
                        {
                            TransactionId = document.Id.ToString(),
                            TransactionTime =document!=null ? Convert.ToInt64((document.CreationTime.Subtract(new DateTime(1970, 1, 1))).TotalSeconds) : (long?)null
                        }
                    };
                ...
                if (isExternalPlatformClient)
                {
                    try
                    {
                        ExternalPlatformHelpers.RollbackTransaction(Convert.ToInt32(externalPlatformType.StringValue), client, 
                            operationsFromProduct, doc[0], WebApiApplication.DbLogger);
                    }
                    catch (Exception ex)
                    {
                        WebApiApplication.DbLogger.Error(ex.Message);
        ...
        private BaseModel JackpotWin(TransactionInput input)
        {
            var clientSession = ClientBll.GetClientProductSession(input.Token, Constants.DefaultLanguageId, null, false);
            using (var documentBl = new DocumentBll(new SessionIdentity(), WebApiApplication.DbLogger))
            {
        ...
        public HttpResponseMessage GetPaymentInfo(int partnerId, GetPaymentInfo input)
        {
            var signature = input.Signature;
            input.Signature = null;
            var secureKey = string.Empty;
