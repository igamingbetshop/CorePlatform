...
{
    [XmlDeserializer]
    public class MicroGamingController : ApiController
    {
        [HttpPost]
        [Route("{partnerId}/api/MicroGaming/ApiRequest")]
        public HttpResponseMessage ApiRequest(int partnerId, BaseInput input)// handle clienttypeid
        {
            ExternalOperation log = null;
            try
            {
                using (var clientBl = new ClientBll(WebApiApplication.Identity, WebApiApplication.DbLogger))
                {
        ...
        private HttpResponseMessage ReturnResponse(Output response, IBaseBll bl, ExternalOperation requestLog, Input input)
        {
        ...
        private HttpResponseMessage ReturnErrorResponse(int errorId, ExternalOperation requestLog, Input input, Exception ex = null)
        {
            using (var baseBl = new PartnerBll(WebApiApplication.Identity, WebApiApplication.DbLogger))
            {
                if (ex != null)
