    ...
    public static class AgentController
    {
        public static ApiResponseBase CallFunction(RequestBase request, SessionIdentity identity, ILog log)
        {
            switch (request.Method)
            {
                ...
                case "CheckTwoFactorPin":
                    return CheckTwoFactorPin(JsonConvert.DeserializeObject<ApiQRCodeInput>(request.RequestData), request.Token, identity, log);
                ...
                case "GetAgentBalance":
                    return GetAgentBalance(identity);
                case "GetAgentData":
        ...
        private static ApiResponseBase CheckTwoFactorPin(ApiQRCodeInput input, string token, SessionIdentity identity, ILog log)
        {
            using (var userBl = new UserBll(identity, log))
            {
                userBl.VerifyTwoFactorPin(input.Pin);
                var session = userBl.UpdateSessionState(token, (int)SessionStates.Active);
                var user = CacheManager.GetUserById(identity.Id);
                var userIdentity = new Session
                {
        ...
        private static ApiResponseBase GetAgentBalance(SessionIdentity identity)
        {
            ...
            agentBalance.CurrencyId = agent.CurrencyId;
            return new ApiResponseBase
            {
        ...
        private static void CheckInputData(UserModel request, BllUser user, UserBll userBl, string languageId, bool isNew = true)
        {
            ...
            if (!request.CountLimits.Select(x => x.Level).SequenceEqual(levels.Where(x => x != request.Level)))
            ...
            if (user.Type == (int)UserTypes.AdminUser)
            {
                ...
                if (currencySetting != null && currencySetting.UserMinLimit.HasValue)
                {
                    ...
                    var i = 1;
                    if (request.LevelLimits.Any(x => { var res = x.Limit > creditMinLimit / i; i *= 2; return res; }))
                ...
                if (partnerSetting != null)
                {
                    ...
                    if (request.CountLimits.Any(x => partnerLimits.First(y => y.Level == x.Level).Count < x.Count || x.Count < 0))
                        throw BaseBll.CreateException(languageId, Constants.Errors.MaxLimitExceeded);
                }
            }
            else
            {
                ...
                if (!parentUserSettings.AllowDoubleCommission || !request.AllowDoubleCommission.HasValue)
                    request.AllowDoubleCommission = false;
                if (!parentUserSettings.AllowOutright || !request.AllowOutright.HasValue)
                    request.AllowOutright = false;
                var parentLimits = JsonConvert.DeserializeObject<List<CountLimit>>(parentUserSettings.CountLimits);
                if (isNew && request.CountLimits.Any(x => parentLimits.First(y => y.Level == x.Level).Count < x.Count))
                    throw BaseBll.CreateException(languageId, Constants.Errors.MaxLimitExceeded);
                countLimitByLevel = parentLimits.First(x => x.Level == request.Level).Count;
            }

            if (request.CalculationPeriod == null ||
           (request.CalculationPeriod.Count > 1 && (request.CalculationPeriod.Contains(1) || request.CalculationPeriod.Contains(-1))) ||
            ...
            using (var userBl = new UserBll(identity, log))
            {
                using (var documentBl = new DocumentBll(userBl))
                {
                    ...
                    if (partnerSetting != null && partnerSetting.NumericValue.HasValue && partnerSetting.NumericValue != 0)
                    {
                        ...
                        CheckInputData(request, user, userBl, identity.LanguageId);
                    }
            ...
            using (var userBl = new UserBll(identity, log))
            {
                using (var documentBl = new DocumentBll(userBl))
                {
                    using (var transactionScope = CommonFunctions.CreateTransactionScope())
                    {
                        ...
                        {
                            userBl.CheckSecurityCode(securityCode);
                            CheckInputData(userModel, user, userBl, identity.LanguageId, false);
                        }
        ...
        public static ApiResponseBase GetTicker(SessionIdentity identity)
        {
            ...
            return new ApiResponseBase
            {
                ResponseObject = CacheManager.GetPartnerTicker(user.PartnerId, identity.LanguageId)
            };
        }
        ...
        private static ApiResponseBase SaveAnnouncement(ApiAnnouncement apiAnnouncement, SessionIdentity identity, ILog log)
        {
            using (var userBl = new UserBll(identity, log))
            {
                ...
                using (var contentBl = new ContentBll(identity, log))
                {
                    ...
                    if (apiAnnouncement.Receivers != null && apiAnnouncement.Receivers.Any())
                    {
                        foreach (var r in apiAnnouncement.Receivers)
                        {
                            var client = CacheManager.GetClientById(r);
                            if (client == null || client.Id == 0 || client.UserId == null || client.UserId != user.Id)
                                throw BaseBll.CreateException(identity.LanguageId, Constants.Errors.ClientNotFound);
                        }
                    }
                    contentBl.SaveAnnouncement(announcement, false);
                    if (announcement.Type == (int)AnnouncementTypes.Ticker)
                    {
                        CacheManager.RemovePartnerTickerFromCache(announcement.PartnerId, identity.LanguageId);
                        Helpers.Helpers.InvokeMessage("RemoveKeyFromCache", string.Format("{0}_{1}", Constants.CacheItems.Ticker, announcement.PartnerId));
                    }
                    return new ApiResponseBase();
                }
            }
        ...
        private static ApiResponseBase UploadProfileImage(string imageData, SessionIdentity identity, ILog log)
        {
            ...
            using var newMemoryStream = CommonFunctions.ResizeImage(bytes, 455, 455);
