...
namespace IqSoft.CP.AgentWebApi.ControllerClasses
{
    ...
    {
        public static ApiResponseBase CallFunction(RequestBase request, SessionIdentity identity, ILog log, HttpRequest httpRequest, IWebHostEnvironment environment)
        {
            switch (request.Method)
            {
                case "CreateClient":
                    return CreateClient(JsonConvert.DeserializeObject<NewClientModel>(request.RequestData), request.SecurityCode, identity, log, httpRequest, environment);
                case "CloneClient":
                    return CloneClient(JsonConvert.DeserializeObject<NewClientModel>(request.RequestData), identity, log, httpRequest, environment);
                case "GetAgentClients":
                    return GetAgentClients(JsonConvert.DeserializeObject<ApiFilterfnClient>(request.RequestData), identity, log);
        ...
        public static ApiResponseBase CreateClient(NewClientModel clientModel, string securityCode, SessionIdentity identity, ILog log, HttpRequest httpRequest, IWebHostEnvironment env)
        {
                ...
                using (var transactionScope = CommonFunctions.CreateTransactionScope())
                {
                    if (clientModel.Count.HasValue && clientModel.Count.Value > 1)
                    {
                        ...
                        for (int i = 0; i < clientModel.Count; ++i)
                        {
                            ...
                            var clientItem = RegisterClient(identity, new ClientRegistrationInput { ClientData = client, IsQuickRegistration = false }, log, env);
                    ...
                    else
                    {
                        ...
                        client = RegisterClient(identity, new ClientRegistrationInput { ClientData = client, IsQuickRegistration = false }, log, env);
        ...
        public static ApiResponseBase CloneClient(NewClientModel clientModel, SessionIdentity identity, ILog log, HttpRequest httpRequest, IWebHostEnvironment env)
        {
            var user = CacheManager.GetUserById(identity.Id);
            if (user.Type == (int)UserTypes.AdminUser)
                throw BaseBll.CreateException(identity.LanguageId, Constants.Errors.NotAllowed);
        ...
        private static Client RegisterClient(SessionIdentity identity, ClientRegistrationInput input, ILog log, IWebHostEnvironment env)
        {
            using (var clientBl = new ClientBll(identity, log))
            {
                input.IsFromAdmin = true;
                return clientBl.RegisterClient(input, env);
            }
        }
            ...
            using (var clientBl = new ClientBll(identity, log))
            {
                using (var userBl = new UserBll(identity, log))
                {
                    ...
                    return new ApiResponseBase
                    {
                        ...
                        {
                            Count = r.Count(),
                            Entities = r.OrderBy(x => x.State == (int)UserStates.Disabled).ThenByDescending(x => x.UserName)
                                        .Skip(filter.TakeCount * filter.SkipCount).Take(filter.TakeCount)
                                        .ToList()
                        }
                    };
                }
