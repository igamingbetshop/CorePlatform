...
{
    [ApiController]
    public class PayBoxController : ControllerBase
    {
        [HttpPost]
        [Route("api/Paybox/Check")]
        public ActionResult Check(ResultInput input)
        {
            try
            {
            ...
            catch (FaultException<BllFnErrorType> ex)
            {
                ...
                Program.DbLogger.Error(new Exception(JsonConvert.SerializeObject(error)));
                return Ok(CommonFunctions.ConvertObjectToXml(error, Constants.HttpContentTypes.ApplicationXml));
            }
            catch (Exception ex)
            {
        ...
        public ActionResult Refund(RefundInput input)
        {
            try
            {
                using var paymentSystemBl = new PaymentSystemBll(new SessionIdentity(), Program.DbLogger);
        ...
        public ActionResult Result(ResultInput input)
        {
        ...
        public ActionResult WithdrawResult(XmlInput xmlInput)
        {
            Program.DbLogger.Info("XMLInput = " + xmlInput.pg_xml);
            try
            {
                ...
                string signature = string.Format("{0};{1};{2}", script, CommonFunctions.GetSortedValuesAsString(input, ";"), secKey);
                input.pg_sig = CommonFunctions.ComputeMd5(signature);
                Program.DbLogger.Info(JsonConvert.SerializeObject(input.pg_sig));
                if (input.pg_sig != sign)
                    throw BaseBll.CreateException(string.Empty, Constants.Errors.WrongPaymentRequest);
        ...
        public ActionResult AddCardResult(XmlInput xmlInput)
        {
            Program.DbLogger.Info("XMLInput = " + xmlInput.pg_xml);
            try
            {
        ...
        {
            using (var partnerBl = new PartnerBll(Program.Identity, Program.DbLogger))
            {
                var paymentSystem = CacheManager.GetPaymentSystemByName(Constants.PaymentSystems.PayBox);
                if (paymentSystem == null)
                    throw BaseBll.CreateException(string.Empty, Constants.Errors.PaymentSystemNotFound);
        ...
        */
        ...
        private static BllPartnerPaymentSetting GetPaymentSetting(int partnerId, string currencyId, string payboxPaymnentSystem)
        {
