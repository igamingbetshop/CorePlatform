...
namespace IqSoft.CP.BLL.Interfaces
{
    public interface IReportBll : IBaseBll
    {
        ...
        List<DepositsInfo> GetDepositsForDashboard(FilterDashboard filter);

        List<WithdrawalsInfo> GetWithdrawalsForDashboard(FilterDashboard filter);
        ...
        BetShopBets GetBetshopBetsPagedModel(FilterBetShopBet filter, string currencyId, string permission);
        ...
        InternetBetsReport GetInternetBetsPagedModel(FilterInternetBet filter, string currencyId, bool checkPermission);

		InternetBetsReport GetBetsForWebSite(FilterWebSiteBet filter);
