...
namespace IqSoft.CP.BLL.Helpers
{
	public static class CustomMapper
	{
		...
		public static PlatformBet ToPlatformBet(this DAL.Bet bet)
		{
			return new PlatformBet
			{
				Id = bet.Id,
				BetDocumentId = bet.BetDocumentId,
				WinDocumentId = bet.WinDocumentId,
				PayDocumentId = bet.PayDocumentId,
				CurrencyId = bet.CurrencyId,
				ProductId = bet.ProductId,
				BetAmount = bet.BetAmount,
				WinAmount = Math.Min(bet.WinAmount, 15000),
				State = bet.State,
				TypeId = bet.TypeId,
				...
				UserId = bet.UserId,
				DeviceTypeId = bet.DeviceTypeId,
				BetTime = bet.BetTime,
				CalculationTime = bet.CalculationTime,
				PayTime = bet.PayTime,
				BetDate = bet.BetDate,
				CalculationDate = bet.CalculationDate,
				PayDate = bet.PayDate,
				BetShopId = bet.CashDeskId == null ? (int?)null : bet.CashDesk.BetShopId
			};
		}
		...
		public static object ToBonusInfo(this Bonu input)
		{
			return new
			{
				...
				IgnoreEligibility = input.IgnoreEligibility,
		...
		public static object ToClientInfo(this Client input)
		{
			return new
			{
				Id = input.Id,
