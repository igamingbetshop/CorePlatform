...
namespace IqSoft.CP.BLL.Services
{
    public class BaseBll : IBaseBll
    {
        ...
        public BaseBll(SessionIdentity identity, ILog log, IWebHostEnvironment env = null, int? timeout = null)
        {
            Identity = identity;
            Log = log;
            ...
            Db = CreateEntities();
            if (timeout != null)
                Db.Database.SetCommandTimeout(timeout.Value);
        }
        ...
        public void SFTPUpload(string content, string path, FtpModel ftpInput)
        {
            try
            {
                ...
                using (var sftpClient = new SftpClient(ftpHost[0], Convert.ToInt32(ftpHost[1]), ftpInput.UserName, ftpInput.Password))
                {
                    ...
                    if (sftpClient.IsConnected)
                    {
                        ...
                        using (var writer = new StreamWriter(stream))
                        {
                            ...
                            sftpClient.UploadFile(stream, Path.GetFileName(path));
                        }
                    }
                }
            }
        ...
        public void UploadFile(string content, string path, FtpModel ftpModel)
        {
            try
            {
                var credentials = new NetworkCredential(ftpModel.UserName, ftpModel.Password);
                var hostDatas = ftpModel.Url.Split(':');

                using var ftpclient = new FtpClient(hostDatas[0], Convert.ToInt32(hostDatas[1]), credentials);
                ftpclient.Credentials = credentials;
                ftpclient.Connect();
                byte[] bytes = Encoding.UTF8.GetBytes(content);
                ftpclient.Upload(new MemoryStream(bytes), path);
        ...
        public void UploadFile(byte[] bytes, string path, FtpModel ftpModel)
        {
            try
            {
                var credentials = new NetworkCredential(ftpModel.UserName, ftpModel.Password);
        ...
        public void RemoveFtpFile(FtpModel ftpModel, string path)
        {
            try
            {
                var credentials = new NetworkCredential(ftpModel.UserName, ftpModel.Password);
                ...
                ftpclient.Credentials = credentials;
                ftpclient.Connect();
                ftpclient.DeleteFile("/coreplatform/website/" + path);
                ftpclient.Disconnect();
            }
            catch (Exception e)
            {
        ...
        protected Translation CreateTranslation(fnTranslation translation)
        {
            ...
            newTranslation.SessionId = SessionId;
        ...
        public static decimal ConvertCurrencyForJob(string fromCurrencyId, decimal fromCurrencyRate, string toCurrencyId, decimal toCurrencyRate, decimal amount)
        {
            if (amount == 0)
                return amount;
            return amount * GetCurrenciesDifferenceForJob(fromCurrencyId, fromCurrencyRate, toCurrencyId, toCurrencyRate);
        }

        public static decimal GetCurrenciesDifferenceForJob(string fromCurrencyId, decimal fromCurrencyRate, string toCurrencyId, decimal toCurrencyRate)
        {
            if (fromCurrencyId == toCurrencyId || toCurrencyRate <= 0)
                return 1;
        ...
        public string ExportToCSV<T>(string fileName, List<T> exportList, DateTime? fromDate, DateTime? endDate, double timeZone, IWebHostEnvironment env)
        {
            var localPath = string.Format("{0}/{1}", env.ContentRootPath, "ExportExcelFiles");
            ...
            if (exportList.Count > maxExcelRowAccount)
            {
                ...
                foreach (var itmList in rootList)
                {
                    ...
                    var task = Task.Run(() =>
                    {
                        ExportExcelHelper.SaveToCSV<T>(itmList, fromDate, endDate, currentDate, timeZone, filePath);
                    });
                    index++;
                    tasks.Add(task);
                }
            ...
            else
            {
                ...
                ExportExcelHelper.SaveToCSV<T>(exportList, fromDate, endDate, currentDate, timeZone, filePath);
        ...
        public static FaultException<BllFnErrorType> CreateException(string languageId, int errorId, decimal? decimalInfo = null, DateTime? dateTimeInfo = null, long? integerInfo = null)
        {
        ...
        public static void CheckIp(List<string> whitelistedIps, string ip)
        {
