...
namespace IqSoft.CP.BLL.Services
{
    public class PermissionBll : BaseBll, IPermissionBll
    {
        ...
        public PermissionBll(SessionIdentity identity, ILog log, IWebHostEnvironment env = null, int? timeout = null)
            : base(identity, log, env, timeout)
        {
        ...
        public CheckPermissionOutput GetPermissionsToObject(CheckPermissionInput input)
        {
            ...
            {
                response.AccessibleObjects = CacheManager.GetUserAccessObjects(input.UserId ?? Identity.Id).Where(
                    x =>
                        x.ObjectTypeId == (int)input.ObjectTypeId &&
                        x.PermissionId == input.Permission)
                    .Select(x => x.ObjectId).AsQueryable();
            }
            return response;
        }

        public CheckPermissionOutput CheckPermission(string checkPermission)
        {
            var permissions = CacheManager.GetUserPermissions(Identity.Id).Where(x => x.PermissionId == checkPermission || x.IsAdmin).ToList();
        ...
        public Role CloneRole(int roleId, string newRoleName)
        {
            ...
            if ((!checkPermissionResult.HaveAccessForAllObjects && !checkPermissionResult.AccessibleObjects.AsEnumerable().Contains(roleId)) || (dbRole.PartnerId.HasValue &&
        ...
        public List<UserRole> GetRoleUsers(int roleId)
        {
            ...
            if (!userAccess.HaveAccessForAllObjects)
                query = query.Where(x => userAccess.AccessibleObjects.AsEnumerable().Contains(x.UserId));
            if (!partnerAccess.HaveAccessForAllObjects)
                query = query.Where(x => partnerAccess.AccessibleObjects.AsEnumerable().Contains(x.User.PartnerId));
        ...
        private void CreateFilterWithPermissions(FilterRole filter)
        {
            ...
            filter.CheckPermissionResuts = new List<CheckPermissionOutput<Role>>
            {
                ...
                new CheckPermissionOutput<Role>
                {
                    ...
                    Filter = x => !x.PartnerId.HasValue || checkPartnerPermission.AccessibleObjects.AsEnumerable().Contains(x.PartnerId.Value)
                }
            };
        }
    }
