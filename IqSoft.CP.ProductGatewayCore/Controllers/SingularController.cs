...
{
    [ApiController]
    public class SingularController : ControllerBase
    {
                ...
                using (var documentBl = new DocumentBll(partnerBl))
                {
                    try
                    {
                        ...
                        response = new AmountResponse()
                        {
                            ...
                            Amount = GetBalanceAmount(documentBl, input.UserId, input.CurrencyId)
                        };
                    }
                    catch (FaultException<BllFnErrorType> fex)
                    {
        ...
        private decimal GetBalanceAmount(DocumentBll documentBl, long clientId, string currency)
        {
            var balance = documentBl.GetObjectBalanceWithConvertion((int)ObjectTypes.Client, clientId, currency).AvailableBalance;
            ...
            using (var clientBl = new ClientBll(new SessionIdentity(), Program.DbLogger))
            {
				using (var documentBl = new DocumentBll(clientBl))
				{
					...
					if (transactionType == OperationTypes.Bet)
						res = clientBl.CreateCreditFromClient(operationsFromProduct, documentBl);
					else
					{
