...
{
    [ApiController]
    public class YSBController : ControllerBase
    {
        ...
        public ActionResult ValidateTicket()
        {
            ...
            {
                Program.DbLogger.Error(faultException.Detail.Message);
        ...
        public ActionResult ApiRequest(int partnerId)
        {
            ...
            try
            {
                ...
                Program.DbLogger.Info("Output: " + JsonConvert.SerializeObject(response));
                return Ok(CommonFunctions.ConvertObjectToXml(response, Constants.HttpContentTypes.ApplicationXml));
            }
            catch (FaultException<BllFnErrorType> faultException)
            {
        ...
        private void GetBalance(ValidationInput input, out ValidationOutput output)
        {
            ...
            using (var clientBl = new ClientBll(new SessionIdentity(), Program.DbLogger))
            {
                ...
                var balance = (clientBl.GetObjectBalanceWithConvertion((int)ObjectTypes.Client,
                       clientId, client.CurrencyId).AvailableBalance / Ratio).ToString("0.##");
        ...
        private void BetConfirmation(ValidationInput input, out ValidationOutput output)
        {
            ...
            using (var documentBl = new DocumentBll(new SessionIdentity(), Program.DbLogger))
            {
                using (var clientBl = new ClientBll(documentBl))
                {
                    ...
                    var balance = (clientBl.GetObjectBalanceWithConvertion((int)ObjectTypes.Client,
                      clientId, client.CurrencyId).AvailableBalance / Ratio).ToString();
                    BaseHelpers.RemoveClientBalanceFromeCache(client.Id);
                    BaseHelpers.BroadcastBalance(client.Id);
                    output.Action = input.Action;
                    output.Elem.Id = input.Elem.Id;
