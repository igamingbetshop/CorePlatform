...
namespace IqSoft.CP.ProductGateway.Controllers
{
    ...
    public class BetGamesController : ControllerBase
    {
        ...
        public ActionResult ApiRequest(int partnerId)
        {
            ...
            try
            {
                ...
                Program.DbLogger.Info("Input: " + responseString);
                    ...
                    switch (input.Method)
                    {
                        ...
                        case BetGamesHelpers.Methods.GetBalance:
                            if (CommonFunctions.ComputeMd5(GetValueFromClassProperty(input) + partnerKey).ToLower() != input.Signature ||
                                client == null)
                                throw BaseBll.CreateException(Constants.DefaultLanguageId, Constants.Errors.WrongHash);
                            response.Parameters.Balance = Convert.ToInt32(BaseBll.GetObjectBalance((int)ObjectTypes.Client, client.Id).AvailableBalance * 100).ToString();
                            break;
                        case BetGamesHelpers.Methods.DoBet:
                            ...
                            if (CommonFunctions.ComputeMd5(GetValueFromClassProperty(betInput) + partnerKey).ToLower() != input.Signature ||
                                client == null)
                                throw BaseBll.CreateException(Constants.DefaultLanguageId, Constants.Errors.WrongHash);
                            var alreadyProcessed = DoBet(betInput, clientSession);
                            response.Parameters.BalanceAfter = Convert.ToInt32(BaseBll.GetObjectBalance((int)ObjectTypes.Client, client.Id).AvailableBalance * 100).ToString();
                                ...
                                using (var transactionScope = CommonFunctions.CreateTransactionScope())
                                {
                                    foreach (var bet in betsInput.Parameters.BetItem)
                                        betsProcessed = DoBet(betsInput.Parameters.GameItem.Id.ToString(), bet.BetId,
                                                              betsInput.Parameters.SubscriptionId, bet.Amount.Value, clientSession);
                        ...
                        case BetGamesHelpers.Methods.CombinationBet:
                            ...
                            var combBetsProcessed = DoBet(gameExtId, combinationId, combinationId,
                                                          combinationBetsInput.Parameters.Amount.Value, clientSession);
                            response.Parameters.BalanceAfter = Convert.ToInt32(BaseBll.GetObjectBalance((int)ObjectTypes.Client, client.Id).AvailableBalance * 100).ToString();
                        ...
                        case BetGamesHelpers.Methods.CombinationWin:
                            ...
                            if (CommonFunctions.ComputeMd5(GetValueFromClassProperty(compinationWinsInput) + partnerKey).ToLower() != input.Signature ||
                                client == null)
                                throw BaseBll.CreateException(Constants.DefaultLanguageId, Constants.Errors.WrongHash);
                            var winsProcessed = DoWin(compinationWinsInput.Parameters, client);
                            response.Parameters.BalanceAfter = Convert.ToInt32(BaseBll.GetObjectBalance((int)ObjectTypes.Client, client.Id).AvailableBalance * 100).ToString();
                        ...
                        case BetGamesHelpers.Methods.DoWin:
                            ...
                            if (CommonFunctions.ComputeMd5(GetValueFromClassProperty(winInput) + partnerKey).ToLower() != input.Signature ||
                                client == null)
                                throw BaseBll.CreateException(Constants.DefaultLanguageId, Constants.Errors.WrongHash);
                            var winProcessed = DoWin(winInput, client);
                            response.Parameters.BalanceAfter = Convert.ToInt32(BaseBll.GetObjectBalance((int)ObjectTypes.Client, client.Id).AvailableBalance * 100).ToString();
                        ...
                        case BetGamesHelpers.Methods.DoPromoWin:
                            ...
                            if (CommonFunctions.ComputeMd5(GetValueFromClassProperty(promoWinInput) + partnerKey).ToLower() != input.Signature ||
                                client == null)
                                throw BaseBll.CreateException(Constants.DefaultLanguageId, Constants.Errors.WrongHash);
                            var promoProcessed = DoPromoWin(promoWinInput, clientSession);
                            response.Parameters.BalanceAfter = Convert.ToInt32(BaseBll.GetObjectBalance((int)ObjectTypes.Client, client.Id).AvailableBalance * 100).ToString();
            ...
            catch (FaultException<BllFnErrorType> fex)
            {
                ...
                Program.DbLogger.Error(string.Format("ErrorMEssage: {0},  Input: {1}", errorDescription, responseString));
            ...
            return Ok(CommonFunctions.ToXML(response));
        }
        ...
        private bool DoBet(BetInput input, SessionIdentity clientSession)
        {
            using (var documentBl = new DocumentBll(clientSession, Program.DbLogger))
            {
        ...
        private bool DoBet(string gameExtId, string betId, string combinationId, int amount, SessionIdentity clientSession)
        {
            using (var documentBl = new DocumentBll(clientSession, Program.DbLogger))
            {
                using (var clientBl = new ClientBll(documentBl))
        ...
        private bool DoWin(BetInput input, BllClient client)
        {
        ...
        private bool DoWin(CombinationBetInputParams input, BllClient client)
        {
            using (var clientBl = new ClientBll(new SessionIdentity(), Program.DbLogger))
            {
                using (var documentBl = new DocumentBll(clientBl))
        ...
        private bool DoPromoWin(PromoWinInput input, SessionIdentity clientSession)
        {
            using (var clientBl = new ClientBll(new SessionIdentity(), Program.DbLogger))
            {
                using (var documentBl = new DocumentBll(clientBl))
