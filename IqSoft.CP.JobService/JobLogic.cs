    ...
    public static class JobBll
    {
        ...
        public static void AwardCashBackBonuses(DateTime lastExecutionTime, ILog log)
        {
            ...
            clientList.ForEach(clientId =>
            {
                ...
                BaseHub.BroadcastBalance(clientId);
        ...
        public static void GiveFixedFeeCommission(ILog log)
        {
            using (var bonusBl = new BonusService(new SessionIdentity(), log))
            {
                bonusBl.GiveFixedFeeCommission(log);
            }
        }
        ...
        public static void UpdateClientWageringBonus()
        {
            using (var db = new IqSoftCorePlatformEntities())
            {
                ...
                foreach (var lb in lostBonuses)
                {
                    ...
                    BroadcastRemoveCache(string.Format("{0}_{1}_{2}_{3}", Constants.CacheItems.ClientBonus, lb.ClientId, lb.BonusId, lb.ReuseNumber ?? 1));
                }
                ...
                foreach (var clientBonus in dbClientBonuses)
                {
                    ...
                    if (clientBonus.TurnoverAmountLeft == 0)
                    {
                        ...
                        BroadcastRemoveCache(string.Format("{0}_{1}_{2}_{3}", Constants.CacheItems.ClientBonus, 
                            clientBonus.ClientId, clientBonus.BonusId, clientBonus.ReuseNumber ?? 1));
                    }
                    else
                    {
        ...
        public static void FinalizeWageringBonus(ILog log)
        {
            ...
            {
                clientList = documentBl.FinalizeWageringBonusDocument(out usersList);
            }
            foreach (var clientB in clientList)
            {
                ...
                BaseHub.BroadcastBalance(clientB.ClientId);
                BroadcastRemoveCache(string.Format("{0}_{1}_{2}_{3}", Constants.CacheItems.ClientBonus, clientB.ClientId, clientB.BonusId, clientB.ReuseNumber ?? 1));
            }
            var userIds = usersList.Distinct();
        ...
        public static void CloseTournaments(ILog log)
        {
            var clients = new List<int>();
            using (var documentBl = new DocumentBll(new SessionIdentity(), log))
            {
                clients = documentBl.CloseTournaments();
            }
            foreach (var c in clients)
            {
                BroadcastRemoveCache(string.Format("{0}_{1}", Constants.CacheItems.ClientBalance, c));
                BaseHub.BroadcastBalance(c);
            }
        }
            ...
            try
            {
                using (var db = new IqSoftCorePlatformEntities())
                {
                    ...
                    if (affiliatePlatforms.Count <= 0)
                        return;
                    var consideredAffiliates = new List<int>();
                    ...
                    using (var affiliateService = new AffiliateService(baseBll))
                    {
                        var clientsByAffiliate = db.Clients.Where(x => affiliatePlatforms.Contains(x.AffiliateReferral.AffiliatePlatform.Id) &&
                                                           ...
                                                           .Select(x => new DAL.Models.Affiliates.AffiliatePlatformModel
                                                           {
                                                               ...
                                                               AffiliateId = x.AffiliateReferral.AffiliateId,
                                                           ...
                                                           .GroupBy(x => new { x.PartnerId, x.AffiliateName, x.AffiliatePlatformId, x.KickOffTime, x.LastExecutionTime, x.StepInHours })
                                                           .ToList();
                        ...
                        {
                            consideredAffiliates.Add(affClient.Key.AffiliatePlatformId);
                            ...
                            switch (affClient.Key.AffiliateName)
                            {
                                case AffiliatePlatforms.Netrefer:
                                    ...
                                    baseBll.UploadFile(content.ToString(), $"{path}/activity_{dateString}.csv", mainFtpModel);
                                    break;
                                ...
                                case AffiliatePlatforms.MyAffiliates:
                                    var sendPostback = CacheManager.GetPartnerSettingByKey(affClient.Key.PartnerId, $"{affClient.Key.AffiliateName}_{Constants.PartnerKeys.AffiliateSendPostback}").StringValue;
                                    if (sendPostback == "0")
                                    {
                                        content.AppendLine(string.Join(",", typeof(AffiliateReportInput).GetProperties().Select(x => x.Name.ToUpper()).ToArray()));
                                        if (newRegisteredClients.Any())
                                        {
                                            content.AppendLine(newRegisteredClients.Aggregate(string.Empty, (current, item) => current +
                                            string.Format("{0},{1},{2},{3},{4},{5},{6}", item.BrandId, item.AffiliateId, item.CustomerId, item.BTag, item.CountryCode,
                                                                                     item.RegistrationDate, item.LanguageId) + Environment.NewLine));
                                        }
                                        fileName = $"/{partner.Name}_signups_{dateString}.csv";
                                        baseBll.SFTPUpload(content.ToString(), fileName, string.Empty, ftpModel);
                                        baseBll.UploadFile(content.ToString(), $"{path}{fileName}", mainFtpModel);
                                        content.Clear();
                                    }
                                ...
                                case AffiliatePlatforms.Affilka:
                                    ...
                                    content.AppendLine(affilkaClientActivies);
                                ...
                                case AffiliatePlatforms.Trackier:
                                    var apiKey = CacheManager.GetPartnerSettingByKey(affClient.Key.PartnerId, AffiliatePlatforms.Trackier + Constants.PartnerKeys.AffiliateApiKey).StringValue;

                                    httpInput = new HttpRequestInput
                                    {
                                        RequestMethod = Constants.HttpRequestMethods.Post,
                                        ContentType = Constants.HttpContentTypes.ApplicationJson,
                                        Url = $"{ftpUrl}/activities",
                            ...
                            var affiliatePlaform = db.AffiliatePlatforms.FirstOrDefault(x => x.Id == affClient.Key.AffiliatePlatformId);
                    ...
                    var notConsideredAffliates = affiliatePlatforms.Where(x => !consideredAffiliates.Contains(x)).ToList();
                    foreach (var aff in notConsideredAffliates)
                    {
                        var affiliatePlaform = db.AffiliatePlatforms.FirstOrDefault(x => x.Id == aff);
                        var upToDate = affiliatePlaform.LastExecutionTime.Value.AddHours(affiliatePlaform.StepInHours.Value);
                        affiliatePlaform.LastExecutionTime = upToDate;
                        if (affiliatePlaform.KickOffTime.Value.AddHours(affiliatePlaform.PeriodInHours.Value) <= upToDate)
                            affiliatePlaform.KickOffTime = affiliatePlaform.KickOffTime.Value.AddHours(affiliatePlaform.PeriodInHours.Value);
                        db.SaveChanges();
        ...
        public static bool CalculateAgentsTurnoverProfit(DateTime toDate, ILog log)
        {
            var currentTime = DateTime.UtcNow;
            ...
            var agentsProfit = new List<CommissionItem>();
            try
            {
                using (var userBll = new UserBll(new SessionIdentity(), log))
                {
                    agentsProfit = userBll.GetAgentTurnoverProfit(fDate, tDate).Where(x => x.TotalProfit > 0).ToList();
                }
                using (var transactionScope = CommonFunctions.CreateTransactionScope())
                {
                    using (var userBll = new UserBll(new SessionIdentity(), log))
                    {
                        ...
                        {
                            using (var clientBll = new ClientBll(documentBll))
                            {
                                foreach (var agentProfit in agentsProfit)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            var userTranferInput = new TransferInput
                                            {
                                        ...
                                        else
                                        {
                                            ...
                                            clientBll.CreateDebitToClient(input, client.Id, client.UserName, documentBll, null);
                                        }
                                    ...
                                    {
                                        log.Error("CalculateAgentsTurnoverProfit_" + JsonConvert.SerializeObject(agentProfit), e);
                                    }
                                }
        ...
        public static bool CalculateAgentsGGRProfit(DateTime toDate, ILog log)
        {
            try
            {
                ...
                var fDate = fromDate.Year * 1000000 + fromDate.Month * 10000 + fromDate.Day * 100;
                var tDate = toDate.Year * 1000000 + toDate.Month * 10000 + toDate.Day * 100;
                            ...
                            using (var documentBll = new DocumentBll(userBll))
                            {
                                foreach (var agentProfit in agentsProfit)
                                {
                                    try
                                    {
                                        ...
                                        var commissionTypes = partnerBll.GetPartnerKey(agent.PartnerId, Constants.PartnerKeys.PartnerCommissionType).
                                            StringValue.Split('|').Select(Int32.Parse).ToList();
                                        var amount = agentProfit.TotalProfit.Value;
                                        if (commissionTypes.Contains((int)AgentCommissionTypes.GGRWithoutTurnover))
                                        {
                                            var item = agentsTurnoverProfit.FirstOrDefault(x => x.RecieverAgentId == agentProfit.RecieverAgentId && x.SenderAgentId == agentProfit.AgentId);
                    ...
                    foreach (var ab in partnerBonuses)
                    {
                        try
                        {
                            ...
                            else if (ab.Bonu.Type == (int)BonusTypes.CampaignWagerCasino || ab.Bonu.Type == (int)BonusTypes.CampaignWagerSport)
                            {
                                ...
                                BaseHub.BroadcastBalance(ab.ClientId);
                                BroadcastRemoveCache(string.Format("{0}_{1}", Constants.CacheItems.NotAwardedCampaigns, ab.ClientId));
                                BroadcastRemoveCache(string.Format("{0}_{1}_{2}_{3}", Constants.CacheItems.ClientBonus, ab.ClientId, ab.BonusId, ab.ReuseNumber ?? 1));
                            }
                            else if (ab.Bonu.Type == (int)BonusTypes.SpinWheel)
                            {
                                ab.Status = (int)ClientBonusStatuses.Active;
                                ab.AwardingTime = DateTime.UtcNow;
                                db.SaveChanges();
                    ...
                    using (var db = new IqSoftCorePlatformEntities())
                    {
                        ...
                        using (var notificationBl = new NotificationBll(clientBl))
                        {
                            ...
                            foreach (var trigger in depositTriggers)
                            {
                                ...
                                {
                                    var currentDate = DateTime.UtcNow;
                                    IqSoft.CP.DAL.AffiliateReferral clientAffiliate = null;
        ...
        public static void ReconsiderDynamicSegments(ILog log)
        {
            ...
            try
            {
                using (var db = new IqSoftCorePlatformEntities())
                {
                    using (var dwh = new IqSoftDataWarehouseEntities())
                    {
                        var triggers = db.JobTriggers.Where(x => x.Type == (int)JobTriggerTypes.ReconsiderSegments).OrderBy(x => x.Id).Take(1000);
        ...
        public static void GiveCompPoints(ILog log)
        {
            try
            {
                        ...
                        using (var documentBl = new DocumentBll(clientBl))
                        {
                            foreach (var client in clients)
                            {
                                if (client.Amount > 0)
                                {
                                    ...
                                    clientBl.CreateDebitToClient(client, client.ClientId.Value, string.Empty, documentBl, null);
                                    ...
                                    clientBl.CreateDebitToClient(coinInput, client.ClientId.Value, string.Empty, documentBl, null);
                                }
                                else
                                {
                                    ...
                                    if (compAccount != null && compAccount.Balance != 0)
                                    {
                                        ...
                                        clientBl.CreateCreditCorrectionOnClient(compCorrectionInput, documentBl, false);
                                    }
        ...
        public static int SettleBets(ILog log)
        {
        ...
        public static void CheckDuplicateClients(ILog log)
        {
            try
            {
                log.Info("CheckDuplicateClients_Start");
                using (var db = new IqSoftCorePlatformEntities())
                using (var clientBl = new ClientBll(new SessionIdentity(), log))
                {
                    var currentDateTime = DateTime.UtcNow;
                    var currentDate = currentDateTime.Year * (long)100000000 + currentDateTime.Month * 1000000 + currentDateTime.Day * 10000 +
                                      currentDateTime.Hour * 100 + currentDateTime.Minute;
                    var triggers = db.JobTriggers.Where(x => x.Type == (int)JobTriggerTypes.CheckDuplicates).OrderBy(x => x.Id).Take(1000);
                    using (var dwh = new IqSoftDataWarehouseEntities())
                    {
                        var matchedData = string.Empty;
                        log.Info("CheckDuplicateClients_triggers_" + triggers.Count());
                        foreach (var t in triggers)
                        {
                            var client = CacheManager.GetClientById(t.ClientId);
                            var similiarClients = new List<int>();
                            if (!string.IsNullOrEmpty(t.Info))
                            {
                                matchedData = JsonConvert.SerializeObject(new { Ip = t.Info });
                                similiarClients = dwh.fn_ClientSession().Where(x => x.ClientId != t.ClientId && x.Ip == t.Info && x.PartnerId == client.PartnerId)
                            ...
                            {
                                var dbMatchedData = dwh.DuplicatedClients.Include(x => x.DuplicatedClientHistories)
                                                                         .FirstOrDefault(x => x.ClientId == t.ClientId && x.DuplicatedClientId == mc);
                                if (dbMatchedData == null)
                                {
                                    dbMatchedData = dwh.DuplicatedClients.Add(new DuplicatedClient
                                    {
                                        ClientId = t.ClientId,
                                        DuplicatedClientId = mc,
                                        CreationTime = currentDateTime
                                    });
                                    dwh.SaveChanges();
                                }
                                if (dbMatchedData.DuplicatedClientHistories?.FirstOrDefault(x => x.DuplicatedData == matchedData) == null)
                                {
                                    dwh.DuplicatedClientHistories.Add(new DuplicatedClientHistory
                                    {
                                        DuplicateId = dbMatchedData.Id,
                                        DuplicatedData = matchedData,
                                        CreationTime = currentDateTime,
                    ...
                    db.JobTriggers.RemoveRange(triggers);
                    db.SaveChanges();
                    log.Info("CheckDuplicateClients_Finish");
                }
            }
            catch (Exception ex)
            {
        ...
        public static void GiveFreeSpin(ILog log)
        {
            var activeFreeSpinBonuses = new List<int>();
            using (var db = new IqSoftCorePlatformEntities())
            {
                activeFreeSpinBonuses = db.ClientBonus.Where(x =>
                    (x.Bonu.Type == (int)BonusTypes.CampaignFreeSpin ||
                    x.Bonu.Type == (int)BonusTypes.CampaignWagerCasino) &&
                    x.Status == (int)ClientBonusStatuses.Active && x.Considered != true).Select(x => x.BonusId).Distinct().ToList();
            }
            var currentDate = DateTime.UtcNow;
            Parallel.ForEach(activeFreeSpinBonuses, bonusId =>
            {
                try
                {
                    using (var db = new IqSoftCorePlatformEntities())
                    {
                        ...
                        if (!bonusProductsByProvider.Any())
                        {
                            clientBonuses.All(x =>
                            {
                                x.Status = (dbBonus.Type == (int)BonusTypes.CampaignFreeSpin ? (int)ClientBonusStatuses.Finished : x.Status);
                                x.Considered = true;
                                return true;
                            });
                            db.SaveChanges();
                        ...
                        else
                        {
                            foreach (var clientBonus in clientBonuses)
                            {
                                ...
                                {
                                    if ((dbBonus.MaxGranted.HasValue && dbBonus.TotalGranted > dbBonus.MaxGranted) ||
                                       (dbBonus.MaxReceiversCount.HasValue && dbBonus.TotalReceiversCount > dbBonus.MaxReceiversCount))
                                    {
                                        clientBonus.Status = (int)ClientBonusStatuses.Expired;
                                        db.SaveChanges();
                                        continue;
                                    }
                                }
                                ...
                                bonusProductsByProvider.ForEach(x =>
                                {
                                        ...
                                        switch (x.GameProviderName)
                                        {
                                            ...
                                            case Constants.GameProviders.RelaxGaming:
                                                x.Products.ForEach(y =>
                                                {
                                                    ...
                                                    freespinModel.BetValues = y.BetValues;
                                                    freespinModel.CoinValue = y.CoinValue;
                                                    if (Integration.Products.Helpers.RelaxGamingHelpers.AddFreeRound(freespinModel, log))
                                                        granted = true;
                                                });
                                                break;
                                    ...
                                    {
                                        log.Error("GiveFreeSpin_ClientBonus_" + clientBonus.BonusId + "_" +
                                            clientBonus.Id + "_" + x.GameProviderName + "_" + ex.Detail?.Id + "_" + ex.Detail?.Message);
                                    }
                                    catch (Exception e)
                                    {
                                        log.Error("GiveFreeSpin_ClientBonus_" + clientBonus.BonusId + "_" +
                                            clientBonus.Id + "_" + x.GameProviderName + "_" + e);
                                    }
                                });
                ...
                {
                    log.Error("GiveFreeSpin_" + bonusId + "_" + e);
                }
            });
        }
        ...
        #region BetsConsideration

        public static void CalculateCompPoints(DateTime lastExecutionTime, ILog log)
        {
            try
            {
                ...
                using (var partnerBl = new PartnerBll(new SessionIdentity(), log))
                {
                    var pk = partnerBl.GetPartnerKey(null, Constants.PartnerKeys.LastCompPointBetId);
                ...
                using (var transactionScope = CommonFunctions.CreateTransactionScope())
                {
                    using (var clientBl = new ClientBll(new SessionIdentity(), log))
                    {
                        using (var partnerBl = new PartnerBll(clientBl))
                        {
                            if (newBets.Any())
                            {
                                clientBl.CalculateCompPoint(newBets);

                                var pk = partnerBl.GetPartnerKey(null, Constants.PartnerKeys.LastCompPointBetId);
                                pk.NumericValue = newBets.Max(x => x.Id);
        ...
        public static void CalculateJackpots(DateTime lastExecutionTime, ILog log)
        {
            try
            {
                long? lastConsideredBetId = null;
                using (var partnerBl = new PartnerBll(new SessionIdentity(), log))
                {
                    var pk = partnerBl.GetPartnerKey(null, Constants.PartnerKeys.LastJackpotBetId);
                    if (pk != null && pk.NumericValue != null)
                        lastConsideredBetId = Convert.ToInt64(pk.NumericValue.Value);
                }
                if (lastConsideredBetId == null) return;
                ...
                using (var transactionScope = CommonFunctions.CreateTransactionScope())
                {
                    using (var clientBl = new ClientBll(new SessionIdentity(), log))
                    {
                        using (var partnerBl = new PartnerBll(clientBl))
                        {
                            if (newBets.Any())
                            {
                                clientBl.IncreaseJackpot(newBets);
