...
namespace IqSoft.CP.MasterCacheWebApi.Helpers
{
    public static class CustomMapper
    {
        ...
        public static ApiLoginClientOutput MapToApiLoginClientOutput(this Affiliate client, double timezone, ClientLoginOut clientLoginOut = null)
        {
            var currency = CacheManager.GetCurrencyById(client.CurrencyId);
            return new ApiLoginClientOutput
            {
            ...
            List<BllFnEnumeration> requestStates, List<PaymentRequestHistoryElement> historyItems)
        {
            return paymentRequests.Join(requestStates, pr => pr.Status, rs => rs.Value, 
             ...
             Select(y => new PaymentRequestModel
             {
                 ...
                 PartnerId = y.paymentRequest.PartnerId,
                 ClientId = y.paymentRequest.ClientId,
                 ...
                 Info = string.IsNullOrEmpty(y.paymentRequest.Info) ? "{}" : 
                    JsonConvert.SerializeObject(JsonConvert.DeserializeObject<IqSoft.CP.Common.Models.PaymentInfo>(y.paymentRequest.Info), new JsonSerializerSettings()
                    {
                 ...
                 FirtName = y.paymentRequest.FirstName,
                 LastName = y.paymentRequest.LastName,
                 GroupId = y.paymentRequest.CategoryId,
        ...
        public static PaymentRequestModel MapToPaymentRequestModel(this fnPaymentRequest paymentRequest, double timeZone)
        {
            return new PaymentRequestModel
            {
                ...
                FirtName = paymentRequest.FirstName,
                LastName = paymentRequest.LastName,
                GroupId = paymentRequest.CategoryId
            };
        }
        ...
        public static List<fnAccount> MapToAccounts(this IEnumerable<AccountModel> accountModels)
        {
            return accountModels.Select(MapToAccount).ToList();
        }
		...
		public static ApiClientBonusItem ToApiClientBonusItem(this fnClientBonus bonus, double timeZone)
		{
            ...
            return new ApiClientBonusItem
            {
                ...
                PlayerId = bonus.ClientId,
                Amount = bonus.BonusPrize,
                State = bonus.Status,
                TurnoverAmountLeft = bonus.TurnoverAmountLeft ?? 0,
                CreationTime = bonus.CreationTime.GetGMTDateFromUTC(timeZone),
                FinalAmount = bonus.FinalAmount,
                TypeId = bonus.BonusType,
                Name = bonus.Name,
                AwardingTime = awardingTime.GetGMTDateFromUTC(timeZone),
                CalculationTime = (bonus.CalculationTime ?? (bonus.AwardingTime == null ? (DateTime?)null : awardingTime.AddHours(bonus.ValidForSpending ?? 0))).GetGMTDateFromUTC(timeZone),
                RemainingCredit = bonus.RemainingCredit,
                WageringTarget = bonus.WageringTarget,
